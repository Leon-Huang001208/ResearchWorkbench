...
from scripts.bootstrap_db import verify_schema, seed_defaults
...
def ingest_bounded_sample(sample_size: int, reporter: MinimalReingestReporter, dry_run: bool = False) -> bool:
            ...
            for sample_file in sample_files:
                try:
                    ...
                    source_doc = SourceDocument(
                        ...
                        title=event_data.get("title", event_data.get("event_type", "Untitled event")),
                    ...
                    if not dry_run:
                        ...
                        if result.status == "success":
                            ...
                            logger.warning(f"Ingestion failed for {sample_file.name}: {result.message}")
                            reporter.stats["ingestion_errors"] += 1
                    
                    reporter.stats["sources_ingested"] += 1
                    
                except Exception as e:
                    logger.error(f"Failed to process sample {sample_file.name}: {str(e)}", exc_info=True)
                    reporter.stats["sources_failed"] += 1
            
            if not dry_run:
                db.commit()
        
        logger.info(f"Bounded sample ingestion completed: {reporter.stats['events_ingested']} events ingested")
        return True
...
def run_rebuild_derived_state(reporter: MinimalReingestReporter, limit: Optional[int], dry_run: bool = False) -> bool:
    ...
    try:
        ...
        with get_db() as db:
            # All four phases
            phase1_rebuild_signals(db, event_repo, signal_service, rebuild_reporter, limit=limit, dry_run=dry_run)
            phase2_recompute_timing_decisions(signal_service, timing_repo, timing_engine_service, rebuild_reporter, limit=limit, dry_run=dry_run)
            phase3_rebuild_outcomes(timing_repo, outcome_service, rebuild_reporter, limit=limit, dry_run=dry_run)
            phase4_regenerate_artifacts(replay_service, rebuild_reporter, limit=limit, dry_run=dry_run)
...
def run_smoke_validation(reporter: MinimalReingestReporter, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        logger.info(f"Validation counts: documents={doc_count}, events={event_count}, signals={signal_count}, decisions={decision_count}")
...
def main():
    ...
    parser.add_argument("--sample-size", type=int, default=20, help="Maximum number of events to ingest (default: 20)")
    parser.add_argument("--skip-bootstrap", action="store_true", help="Skip database bootstrap step if already done")
    parser.add_argument("--dry-run", action="store_true", help="Only test the flow without modifying the database")
    args = parser.parse_args()
    
    reporter = MinimalReingestReporter()
    logger.info("🚀 Starting minimal re-ingestion bootstrap (Issue #27 recovery path)")
    logger.info(f"Configuration: sample-size={args.sample_size}, skip-bootstrap={args.skip_bootstrap}, dry-run={args.dry_run}")
