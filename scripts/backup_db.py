...
def parse_args():
    ...
    parser.add_argument("--sqlite-path", help="SQLite database path (for SQLite backups)")
...
def backup_postgres(database_url: str, output_path: Path) -> bool:
    ...
    try:
        ...
        logger.error("PostgreSQL backup failed", returncode=e.returncode, stderr=e.stderr)
        return False
...
def backup_sqlite(sqlite_path: Path, output_path: Path) -> bool:
    ...
    try:
        ...
        logger.error("SQLite backup failed", returncode=e.returncode, stderr=e.stderr)
        return False
...
def cleanup_old_backups(backup_dir: Path, retention_days: int):
    ...
    for backup_file in backup_dir.glob("*.sql"):
        try:
            ...
            logger.warning(
                "Could not process backup file for cleanup", file=backup_file.name, error=str(e)
            )
