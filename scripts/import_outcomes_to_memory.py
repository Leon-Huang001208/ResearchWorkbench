...
try:
    ...
    for outcome in outcomes:
        try:
            ...
            episode = MarketEpisode(
                ...
                outcome_return=float(outcome['outcome_return']) if outcome['outcome_return'] is not None else 0.0,
                outcome_excess_return=float(outcome['outcome_excess_return']) if outcome['outcome_excess_return'] is not None else 0.0,
            ...
            imported_count += 1
            print(f"  ✓ Imported episode for {episode.event_type}: return={episode.outcome_return:.2%}, excess={episode.outcome_excess_return:.2%}")
    ...
    if imported_count > 0:
        ...
        for event_type in sorted(event_types):
            ...
            print(f"  - {event_type}: {summary['sample_size']} samples, "
