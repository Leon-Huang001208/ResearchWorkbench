...
def parse_args():
    ...
    parser.add_argument(
        "--sqlite-path",
        help="SQLite database path (for SQLite restores)"
    )
    parser.add_argument(
        "--drop-existing", "-f",
        action="store_true",
...
def restore_postgres(database_url: str, backup_file: Path, drop_existing: bool, create_db: bool) -> bool:
    ...
    if drop_existing:
        ...
        try:
            ...
            if create_db:
                ...
                logger.error(
                    "Failed to drop existing database",
    ...
    try:
        if is_compressed:
            ...
            p2 = subprocess.Popen(psql_cmd, env=env, stdin=p1.stdout, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
            p1.stdout.close()
