...
线上研究或产品发布，也没有触碰桌面原目录。后续交付步骤在同步当前主分支并复验后合并本地分支、
推送功能分支并创建 PR。macOS Research Web 测试不能作为桌面或 Windows
验证证据。

## 去独有化决策

- 保留开发基线中的五个通用 Skill 和四个 Workflow 的稳定 ID、历史版本与职责；新增五个窄边界
  专用 Skill。与当前主分支并行加入的“因子库研究”合并后，目录为 11 Skill + 4 Workflow。
...
| 范围 | 主要文件 | 实现提交 |
| --- | --- | --- |
| 进程入口检查 | `capabilities/catalog.py`、`test_capabilities_safety.py` | `24e93e8f`、`61381f96`、`16d03129`、`a2dbfb99` |
| 声明式种子、四项专用 Skill、共享协议与会话快照 | `capabilities/seeds.py`、`skills/_shared/`、四个专用 Skill、能力/会话测试 | `c6642248`、`e7be62c0`、`0123a5a7` |
| 研报增量 Skill、校验与 SVG 脚本 | `skills/sell-side-report-reader/`、`test_sell_side_report_skill.py` | `c1613b36`、`81172738`、`48b012eb`、`92a3802b`、`0b5ce764` |
...
- 同步当前主分支后的能力聚焦 pytest：`192 passed, 2 skipped, 1 warning`。覆盖目录、元数据、导入/导出、资源哈希、
...
- JavaScript/UI：`48 passed`。新增测试通过项目 Python 环境实例化产品
  `CapabilityCatalog`，从 `list(kind="skill")` 获取真实种子目录，再交给现有页面函数验证当前
  11 个内置 Skill 的唯一性、分类、中文/技术 ID 搜索、详情，以及不存在
...
- `scripts/check_doc_sync.py --base origin/master` 通过，架构门禁无 violation；能力拓扑复用既有发布、
