...
- 主机事件创建使用去重键与周期序号的确定性 ID；仓储在 savepoint 中处理并发主键冲突并返回现有未解决事件。已解决事件不会被重开，后续压力周期生成新的事件 ID。
...
| 命令 | 实际结果 |
| --- | --- |
| `python -m pytest tests/unit/test_resource_monitor_alert_service.py tests/unit/test_resource_monitor_runtime.py tests/unit/data_layer/repositories/test_monitoring_repository.py -q` | 29 passed |
