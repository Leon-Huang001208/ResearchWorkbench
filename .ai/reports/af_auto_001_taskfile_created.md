...
成功将 AF-AUTO-001 提案转换为结构化的 JSON 任务文件，使用与 AF-AUTO-000 相同的 schema。
...
| 文件 | 路径 | 状态 |
|------|------|------|
| 任务文件 | `.ai/tasks/task_af_auto_001.json` | ✅ 已创建 |
| 报告文件 | `.ai/reports/af_auto_001_taskfile_created.md` | ✅ 已创建 |
...
1. **af-auto-001-01**: Fix Failing Tests
   - 修复 AF-AUTO-000 基线运行中识别的失败测试，使测试套件足够稳定
   - 对失败测试按根本原因分类
   - 创建详细修复报告

2. **af-auto-001-02**: Add Quick-Win Tests (Phase 1)
   - 为关键但测试较少的服务添加快速胜利测试
   - 依赖: af-auto-001-01
   - 测试范围: signal_service, failure_memory_service, outcome_service, search_service, report_generator

3. **af-auto-001-03**: Complete Reasoning Layer TODOs
   - 实现 AF-AUTO-000 期间识别的推理层剩余 TODO 项
   - LLM 假设生成、断言查询、事件查询、时间相关性检查
   - 需要研究: true

### 中优先级任务 (3个)

4. **af-auto-001-04**: Phase 2 - Critical Services Test Coverage
   - 为高风险核心服务添加测试
   - 依赖: af-auto-001-02
   - 测试范围: paper_trading_service, monitoring_service, replay_service, closed_loop_service, governance_service

5. **af-auto-001-05**: Phase 3 - Knowledge Layer Test Coverage
   - 为知识层模块添加测试
   - 依赖: af-auto-001-04
   - 测试范围: entity_resolution, assertions, events, graph_projection, retrieval

6. **af-auto-001-06**: Phase 4 - Reach 75 Percent Coverage
   - 添加剩余测试，达到 75% 覆盖率目标
   - 依赖: af-auto-001-05
