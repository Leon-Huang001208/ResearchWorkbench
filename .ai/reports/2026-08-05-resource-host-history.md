...
- 在写入时按 1500 条一批循环清理所有超过 24 小时的同类记录；查询严格排除 24 小时保留边界，资源事件及其他监控指标不会被删除。
...
| 命令 | 实际结果 |
| --- | --- |
| `python -m pytest tests/unit/test_resource_host_history_service.py tests/unit/test_monitoring.py -q` | 56 passed |
