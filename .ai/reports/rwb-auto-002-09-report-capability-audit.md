# RWB-AUTO-002-09: 当前报告生成能力审计报告

## 执行摘要

本次审计发现 Research Workbench 已具备较为完整的报告生成基础设施，包括：
...
### 7.1 为 RWB-AUTO-002-10~15 的扩展点

基于现有架构，建议按以下方式扩展：

```
RWB-AUTO-002-10: 设计模板驱动报告数据模型
    ↓ 复用现有契约，仅需补充 PPTX 相关
RWB-AUTO-002-11: 实现 DOCX 模板上传与渲染
    ↓ 直接复用 WordProjection.save_from_template()
RWB-AUTO-002-12: 实现 PPTX 模板上传与渲染
    ↓ 新增 PPTXProjection（类似 WordProjection）
RWB-AUTO-002-13: 添加模板管理 API
    ↓ 复用 TemplateManager + 新增 API 端点
RWB-AUTO-002-14: 添加 Web UI
    ↓ 基于 API 构建
RWB-AUTO-002-15: 集成现有 ReportGenerator
    ↓ 桥接旧有快照数据到新模板系统
```
...
### 8.1 第一阶段：增强模板管理 API（RWB-AUTO-002-13 可先做）
...
### 8.2 第二阶段：DOCX 模板上传（RWB-AUTO-002-11）
...
### 8.3 第三阶段：PPTX 支持（RWB-AUTO-002-12）

**新增模块**:
- PPTXProjection（参考 WordProjection）

### 8.4 第四阶段：集成现有 ReportGenerator（RWB-AUTO-002-15）
