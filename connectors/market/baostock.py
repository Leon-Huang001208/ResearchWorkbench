"""BaoStock MarketDataConnector — 将现有 BaoStockAdapter 包装为 MarketDataConnector.

Wrapper-first 策略：内部委托给 data_layer/crawlers/baostock/，
不立即重写内部逻辑。

BaoStock 与 AKShare 使用相同的 MarketData / StockInfo dataclass，
因此可复用 data_layer.normalizers.akshare_market 的标准化函数。
...
from data_layer.crawlers.baostock.base import BaoStockAdapter, BaoStockConfig
from data_layer.normalizers.akshare_market import normalize_market_data, normalize_stock_info

logger = get_logger(__name__)


class BaostockMarketConnector(MarketDataConnector):
    """BaoStock 市场数据连接器.

    包装 BaoStockAdapter，提供统一的 MarketDataConnector 接口。
    BaoStock 是免费开源的 A 股数据源，无需 API Key，适合离线或低预算场景。
    """
    ...
    def __init__(self, config: Dict[str, Any] | None = None):
        ...
        Args:
            config: 连接器配置字典。可包含：
                ...
                - default_frequency: 默认 K 线频率（默认 "d"）
                - default_adjustflag: 默认复权方式（默认 "3"=不复权）
        """
        super().__init__(config)
        cfg = config or {}
        baostock_config = BaoStockConfig(
            enable_cache=cfg.get("enable_cache", True),
            timeout=cfg.get("timeout", 30.0),
            max_retries=cfg.get("max_retries", 3),
            default_frequency=cfg.get("default_frequency", "d"),
            default_adjustflag=cfg.get("default_adjustflag", "3"),
        )
        self._adapter = BaoStockAdapter(config=baostock_config)
    ...
    def datasets(self) -> List[str]:
        return ["stock_daily", "stock_master"]
    ...
    def health_check(self) -> HealthStatus:
        ...
        通过尝试获取 600519.SH 最近 5 天数据来验证连通性。
        """
        try:
            ...
            if status == "healthy":
                ...
                logger.warning("baostock_health_unavailable", extra={"result": result})
                return HealthStatus.UNAVAILABLE
            self._health = HealthStatus.DEGRADED
            logger.warning("baostock_health_degraded", extra={"result": result})
            return HealthStatus.DEGRADED
        except Exception as e:
            self._health = HealthStatus.UNAVAILABLE
            logger.error("baostock_health_failed", extra={"error": str(e)}, exc_info=True)
            return HealthStatus.UNAVAILABLE
    ...
    def discover(self, dataset: str, **params: Any) -> List[DiscoveryItem]:
        ...
        Args:
            dataset: 数据集标识（stock_daily / stock_master）.
            **params:
        ...
        if dataset == "stock_daily":
            ...
            return [
                DiscoveryItem(
                    ...
                    item_type="stock_master",
                    params={},
                    description="fetch A-share stock list from BaoStock",
                )
            ]

        logger.warning(
            "baostock_unknown_dataset",
    ...
    def fetch(self, dataset: str, item: DiscoveryItem, **params: Any) -> RawObject:
        ...
        if item_type == "stock_daily":
            ...
            raise ValueError(
                ...
                f"Supported: stock_daily, stock_master"
            )
        ...
        for row in table.rows:
            if dataset == "stock_daily" or table.table_name == "stock_daily":
                payload = MarketBarPayload(
                    ...
                    adjustment=table.metadata.get("adjustflag", "3"),
                )
    ...
    def _fetch_stock_daily(self, item: DiscoveryItem, params: Dict[str, Any]) -> RawObject:
        ...
        frequency = item.params.get("frequency")
        adjustflag = item.params.get("adjustflag") or "3"
        ...
        source_uri = f"baostock://market/stock_daily/{symbol}"
    ...
    def _fetch_stock_master(self, item: DiscoveryItem, params: Dict[str, Any]) -> RawObject:
        ...
        source_uri = "baostock://market/stock_master"
