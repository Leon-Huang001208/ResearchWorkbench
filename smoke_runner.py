...
def test_smoke() -> bool:
    ...
    # 2. 启动服务
    print("\n🔧 2. 启动API服务...")
    proc = subprocess.Popen("uvicorn app.api.main:app --port 8001", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    time.sleep(5)
    ...
    try:
        ...
        try:
            ...
            if resp.status_code != 200:
                ...
                return False
            data = resp.json()
            if data.get("canonical_id") != "600000" or data.get("price_volume", {}).get("close_price") is None:
                print("❌ 资产分析返回数据不完整")
                return False
