...
class EventAutoSignalGenerator:
    ...
    def process_approved_events(self) -> int:
        ...
        for event in approved_events:
            try:
                logger.info(f"Generating signal for approved event: {event.event_id} - {event.title}")
                ...
                logger.info(f"Successfully generated {len(signals)} signals for event {event.event_id}")
            except Exception as e:
                logger.error(f"Failed to generate signals for event {event.event_id}: {str(e)}", exc_info=True)
