...
class ThesisGeneratorService:
        ...
        for json_file in DATA_DIR.glob("*.json"):
            try:
                with open(json_file, "r", encoding="utf-8") as f:
                    ...
                    logger.info(f"Loaded industry graph: {graph.graph_id} ({len(graph.nodes)} nodes, {len(graph.edges)} edges)")
    ...
    def _find_starting_nodes(self, event: CanonicalEvent, graph: IndustryGraph) -> List[IndustryNode]:
        """找到和事件匹配的起始节点"""
        starting_nodes = []
        ...
        for node in graph.nodes:
            if not node.is_company:
                ...
                matches_industry = any(
                    ii in node_industry or node_industry in ii
                    for ii in impacted_industries
                )
