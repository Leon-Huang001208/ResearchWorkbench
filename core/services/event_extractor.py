...
_A_SHARE_PATTERN = re.compile(
    r"\b(\d{6})\.(SH|SZ)\b", re.IGNORECASE
)

# ── 事件类型关键词字典 ────────────────────────────────
_EVENT_TYPE_KEYWORDS: dict[str, list[str]] = {
    "earnings": [
        "业绩", "营收", "净利润", "一季报", "中报", "三季报", "年报",
        "财报", "盈利", "亏损", "扭亏", "增收",
    ],
    "policy": [
        "政策", "法规", "监管", "批复", "补贴", "减税", "新规",
        "国务院", "发改委", "证监会", "银保监", "降准", "降息",
    ],
    "product": [
        "发布", "上市", "量产", "获批", "新品", "投产", "突破",
        "技术突破", "研发",
    ],
    "merger_acquisition": [
        "收购", "并购", "重组", "合并", "分拆", "借壳",
    ...
    "supply_chain": [
        "供应", "断供", "缺货", "涨价", "降价", "产能", "库存",
    ],
    "macro": [
        "GDP", "CPI", "PMI", "利率", "汇率", "通胀", "通缩",
        "降息", "加息", "社融",
    ],
}
...
# ── 影响方向关键词 ──────────────────────────────────
_BULLISH_KEYWORDS = [
    "利好", "增长", "上涨", "突破", "超预期", "新高", "获批",
    "放量", "强势", "增持", "上调",
]
_BEARISH_KEYWORDS = [
    "利空", "下跌", "下滑", "不及预期", "亏损", "暴雷", "减持",
    "下调", "退市", "违规",
]
