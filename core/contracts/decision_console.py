...
class DecisionAction(BaseModel):
    ...
    action_type: Literal["approve", "watch", "reject", "defer", "revise_thesis", "add_to_paper_portfolio"] = Field(
        description="Type of decision action"
    )
    action_by: str = Field(description="Identifier or name of the analyst who took the action")
    action_at: datetime = Field(default_factory=datetime.utcnow, description="Timestamp when the action was taken")
    rationale: str = Field(description="Rationale or justification for the action")
    changes: Optional[Dict[str, str]] = Field(default=None, description="Optional dictionary of changes made")
...
class AnalystDecision(BaseModel):
    ...
    candidate_id: str = Field(description="Unique identifier for the candidate (signal, portfolio candidate, or proposal)")
    candidate_type: Literal["signal", "portfolio_candidate", "proposal"] = Field(description="Type of candidate")
    previous_status: str = Field(description="Previous status of the candidate before this decision")
    final_action: DecisionAction = Field(description="The final decision action taken")
    revision_history: List[DecisionAction] = Field(default_factory=list, description="List of previous decision actions")
...
class DecisionWorkspace(BaseModel):
    ...
    status: Literal["open", "in_review", "closed"] = Field(default="open", description="Status of the workspace")
    candidate_ids: List[str] = Field(default_factory=list, description="List of candidate IDs in this workspace")
    team_id: Optional[str] = Field(default=None, description="Optional team ID")
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Timestamp when the workspace was created")
    created_by: str = Field(description="Identifier or name of the user who created the workspace")
    closed_at: Optional[datetime] = Field(default=None, description="Timestamp when the workspace was closed (if applicable)")
...
class PostMortemRecord(BaseModel):
    ...
    realized_outcome: Literal["win", "loss", "draw", "pending"] = Field(description="Realized outcome")
    outcome_metrics: Dict[str, float] = Field(default_factory=dict, description="Dictionary of outcome metrics")
    learning_points: str = Field(description="Key learning points from the post-mortem")
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Timestamp when the post-mortem was created")
    updated_at: datetime = Field(default_factory=datetime.utcnow, description="Timestamp when the post-mortem was last updated")
    linked_signal_accuracy: Optional[float] = Field(default=None, description="Optional accuracy score of the linked signal")
