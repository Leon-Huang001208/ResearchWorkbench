...
class DocumentQuality(BaseModel):
    ...
    source_reliability_level: SourceReliabilityLevel = Field(
        default=SourceReliabilityLevel.UNKNOWN,
        description="来源可信度层级"
    )
    subjectivity_level: SubjectivityLevel = Field(
        default=SubjectivityLevel.MIXED,
        description="主观性层级"
    )
    fact_opinion_ratio: Optional[float] = Field(
        default=None,
        ge=0.0, le=1.0,
        description="事实观点比例（1.0=纯事实，0.0=纯观点）"
    )
    research_usability_score: Optional[float] = Field(
        default=None,
        ge=0.0, le=1.0,
        description="研究可用性评分（1.0=最有用）"
    )
...
class DocumentTimeliness(BaseModel):
    ...
    publish_time_precision: Optional[str] = Field(
        default=None,
        description="发布时间精度（day/hour/minute/second）"
    )
...
class DocumentProcessingMeta(BaseModel):
    ...
    status: DocumentProcessingStatus = Field(
        default=DocumentProcessingStatus.RAW,
        description="处理状态"
    )
...
class DocumentV1(BaseModel):
    ...
    quality: DocumentQuality = Field(
        default_factory=DocumentQuality,
        description="质量评分"
    )

    # 证据概况
    evidence_profile: DocumentEvidenceProfile = Field(
        default_factory=DocumentEvidenceProfile,
        description="证据概况"
    )

    # 时效性
    timeliness: DocumentTimeliness = Field(
        default_factory=DocumentTimeliness,
        description="时效性信息"
    )

    # 处理状态
    processing: DocumentProcessingMeta = Field(
        default_factory=DocumentProcessingMeta,
        description="处理元数据"
    )

    # 审核状态
    review: DocumentReview = Field(
        default_factory=DocumentReview,
        description="审核信息"
    )
...
class DocumentEventV1(BaseModel):
    ...
    doc_id: str = Field(description="文档ID")
    canonical_event_id: Optional[str] = Field(default=None, description="规范事件ID（链接到canonical_event表）")
