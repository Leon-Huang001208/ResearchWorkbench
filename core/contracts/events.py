...
class CanonicalEvent(BaseModel):
    ...
    event_time: datetime | None = Field(
        default=None, description="Timestamp of the event (if available)"
    )
    ...
    raw_text: Optional[str] = Field(
        default=None, description="Raw text of the event/source (if available)"
    )

    # Extracted content
    extracted_assertions: list[dict] = Field(
        default_factory=list, description="List of assertions extracted from the event"
    )

    # Impact targeting
    impacted_industries: list[str] = Field(
        default_factory=list, description="List of industries impacted by this event"
    )
    impacted_symbols: list[str] = Field(
        default_factory=list, description="List of ticker symbols impacted by this event"
    )
    ...
    needs_review: bool = Field(
        default=True, description="Legacy field: whether the event needs review"
    )
    entities: list[dict] = Field(default_factory=list, description="Legacy field: list of entities")
    assertions: list[dict] = Field(
        default_factory=list, description="Legacy field: list of assertions"
    )
    evidence_spans: list[dict] = Field(
        default_factory=list, description="Legacy field: list of evidence spans"
    )
    ...
    reviewer: str | None = Field(
        default=None, description="Legacy field: identifier of the reviewer (if applicable)"
    )
    reviewed_at: datetime | None = Field(
        default=None,
        description="Legacy field: timestamp when the event was reviewed (if applicable)",
    )
