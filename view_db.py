...
def query_latest_events(limit=10):
    ...
    print("-" * 150)
    result = db.execute(text(f"SELECT event_id, event_type, impact_direction, confidence, event_time, summary FROM canonical_event ORDER BY created_at DESC LIMIT {limit}"))
    for row in result:
        ...
        print(f"{row.event_id:<30} {row.event_type:<10} {row.impact_direction:<8} {row.confidence:<6} {event_time:<25} {summary}")
...
def query_latest_documents(limit=10):
    ...
    print("-" * 150)
    result = db.execute(text(f"SELECT doc_id, source_type, source_name, published_at, title FROM source_document ORDER BY created_at DESC LIMIT {limit}"))
    for row in result:
        published_at = row.published_at.isoformat() if row.published_at else "N/A"
        title = row.title[:50] + "..." if row.title else ""
        title = title[:50] + "..." if len(title) > 50 else title
        print(f"{row.doc_id:<30} {row.source_type:<10} {row.source_name:<15} {published_at:<25} {title}")
...
def query_stock_data():
    ...
    print("-" * 100)
    try:
        result = db.execute(text("SELECT canonical_id, as_of FROM asset_snapshot ORDER BY as_of DESC LIMIT 10"))
        for row in result:
