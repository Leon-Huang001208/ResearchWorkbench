...
* 文件大小不超过 25 MB。
* 单次请求音频总时长不超过 120 分钟，仅统计纯音频时长，视频内嵌音频不计入统计。
...
* 回传 `encrypted_content`内容需有效，篡改或无法还原时返回错误：`Invalid signature`。
* `encrypted_content`优先级高于 `reasoning_content`，回传 `encrypted_content`时，将忽略 `reasoning_content`内容。
