...
工作台侧栏仅保留“系统”入口，默认打开“系统中心”，可通过同一页面内的“系统配置”标签进入既有配置工作台；接口路径和配置字段均未改变。两个页面共用完整内容轨道，因此页内“系统中心 / 系统配置”导航在切换前后保持相同左缘。旧版本地导航值 `resource-monitor`、`config` 会分别迁移为系统入口及对应子标签。资源页在正常采样且无待处理异常时不显示成功徽标；仅在存在未恢复异常或采样不可用时显示提示。异常历史仍使用上述 `resource-events` API，状态、严重度筛选器为深色单选浮层，选择、点击外部或按 `Escape` 会关闭浮层。
