...
Issue #42 要求为 AlphaFoundry v1 设计统一的文档 schema，使所有来源类型（电报、新闻、研报、公众号、会议纪要等）都能映射到同一个主模型。
...
Issue #42 "统一 Document Schema" 已完整实施。这为 AlphaFoundry v1 奠定了坚实的基础，使系统能够统一处理所有来源类型的文档，并为后续的采集、解析、检索、研报生成和回测功能提供了数据基础设施。
