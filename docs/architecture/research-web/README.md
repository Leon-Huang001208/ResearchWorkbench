...
研究布局、能力中心与架构更新检查已实施。当前 Web 包含研究台按需数据入口、独立资产观察、Claw 具体报告 Workflow、会话快照交接、实际产物及只读“运行与用量”聚合；DataHub 同时迁入天软 CJPY 的四项已实现能力，但缺少本机依赖或授权时仍不可调用。东方财富基金和财联社是当前无需专业配置即可真实调用的来源。逐项变更见 [迭代核对](review-record.md)。图形通过不替代产品、数据覆盖或真实连接审查。
...
不新增 PostgreSQL 前置条件、Evidence/Claim、第二研究引擎、外部 MCP、线上技能市场或桌面适配。研究台市场页是按需查询和快照入口，不是旧市场首页或后台行情管线。通用 Workflow 是 DSH 读取的研究步骤模板；具体报告 Workflow 还持有模板、Excel 底稿、版本、日程和交付契约，但同样不另建 Agent 执行器。
