...
Tool 目录只读展示原有 9 个研究/控制工具与 13 个 `datahub_*` 业务数据工具，共 22 项。新数据 Tool 使用子系统前缀而非产品品牌，因此将来产品改名不需要迁移研究协议；`af_public_data` 只保留为 deprecated 兼容项。当前可选择的 Tool 为 `af_run_script`、`web_search`、兼容 `af_public_data`，以及已存在可调用 Provider 的 `datahub_get_fund_data`、`datahub_search_news`；其他 `datahub_*` 能力可以浏览，但会明确显示没有已适配来源，不能运行。
