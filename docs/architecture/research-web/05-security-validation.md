...
黄金快照存储在读取前拒绝目录或文件符号链接，限制为 2 MiB，使用严格 schema 和内容 revision，并同目录原子替换。旧 V0 结构只在识别到明确字段时迁移，原文件先保留为 `snapshot.legacy-v0.json`；已存在备份时拒绝覆盖。Bot 上下文另设 80,000 字符上限，默认解释预设不暴露工具。
