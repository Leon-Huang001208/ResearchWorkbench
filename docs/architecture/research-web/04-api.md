...
Windows 专项作业实际启动 `127.0.0.1:8088`，读取 `/local-integrations` 并完成一次幂等探测轮询。该验证覆盖服务初始化与接口生命周期，不新增 API，也不将未安装厂商软件的 runner 投影为可调用。服务启动使用与生产相同的 DSH 认证控制文件读取器；CI 占位元数据不表示 DSH 或厂商服务在线。
