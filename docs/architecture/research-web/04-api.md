...
路由由源码声明、架构清单与重启后的 8088 OpenAPI 双向核对：当前有 127 个唯一 HTTP 操作、129 项源码声明（包括根页）。其中报告运行详情与取消各保留一条兼容声明，因此声明数不能当作唯一接口数。`report_workflow_routes.py` 提供具体报告 Workflow 的资源、版本、Provider 探测、运行、重试、交付和日程接口；`operations.py` 只聚合真实运行证据。目录与消息使用当前原生能力版本契约；API 不是旧 `/api/research-runs`。
...
- `GET /local-integrations` 返回本机集成的安全四维状态；`POST /local-integrations/probes` 保持无副作用发现。`POST /local-integrations/verifications` 要求 `Idempotency-Key`，目标白名单仅含 Excel、Word、PowerPoint 与 Wind Excel，返回 202 与独立任务 ID；两个查询接口只返回安全化状态或错误。真实验证使用服务创建的受管临时文件、独立进程组和 180 秒预算，不接受任意本机路径。探测与验证均不返回命中路径、注册表值、命令参数、环境变量或厂商秘密。
