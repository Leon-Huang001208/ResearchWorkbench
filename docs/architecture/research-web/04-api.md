...
- `GET /local-integrations` 返回本机集成的安全四维状态；`POST /local-integrations/probes` 要求 `Idempotency-Key` 并返回 202 与独立任务 ID，查询接口只返回安全化状态或错误。v0 探测不启动软件，不返回命中路径、注册表值、命令参数或环境变量。
