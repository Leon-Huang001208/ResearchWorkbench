...
- DSH 固定到提交 `c919b2a460753859665db3f60143d525fb9140cf` 与 `pnpm@11.7.0`，按版本写入
  用户私有运行目录并核对构建闭包；嵌套构建通过用户私有 Corepack shim 解析固定 pnpm，不依赖全局安装；
  来源、提交、工作树或闭包不符时关闭失败。
- CJPY 闭合制品目录通过仓库属性保持 checkout 字节稳定，Windows 包装器传播安装器真实失败退出码，避免
  平台换行转换或批处理变量展开把失败误报为成功。
...
- Windows DSH checkout 与工作树校验统一使用 `core.symlinks=false`，固定提交中的 Git symlink 以
  Git 支持的普通文件形式检出时不会被误判为篡改；clone、checkout 与 status 现在还统一使用命令级
  长路径和换行配置，避免长 staging 路径在原生 Windows 上误报 `dsh_worktree_modified`，其他已跟踪
  文件修改仍关闭失败。
- Windows Node 构建环境允许列表补入标准 PowerShell/Program Files 工具链发现路径，使 `node-gyp`
  能定位已安装的 Visual Studio 2022 C++ Build Tools；应用密钥仍不会进入安装子进程。安装文档同步说明
  固定 DSH 的 `fs-ext` 在 Windows 上需要该系统工具链。
- Windows 固定 DSH 构建改用同一私有运行根目录下的短 staging 名称，避免 pnpm 的深层 `fs-ext`
  路径触发 MSBuild FileTracker 的非法路径异常；中断恢复、提交/闭包校验和原子发布保持不变。
...
- 默认启用暴露的 Windows 启动回归已修正：MCP staging、安装 payload、清单与确认令牌私有目录不再把 Windows `st_mode` 投影误当 ACL；目录类型、符号链接和重解析点检查保持关闭失败，POSIX 仍校验 group/other 权限及所有者。
...
- 数据适配器与 iFinD 包入口改为按需装载；Research Web 使用 HTTP iFinD 时不再初始化 CNINFO、数据库或本地 SDK 依赖，项目 `.venv` 无 `psycopg2` 也可启动服务管理器。
...
- 恢复桌面一期 `点评生产`、`报告生产` 导航基线，FinGPT 继续作为增量入口。
...
### Research Web 数据源分类工作台 · 2026-09-09

- 数据源设置将 21 个远程来源按专业数据源、API 数据源和公开来源分组为卡片工作台；顶部汇总真实连接状态，并提供跨分类搜索、状态筛选和既有 DataHub 目录入口。
- 数据源详情改为可关闭的响应式配置抽屉，继续复用原配置、检测、移除和秘密不回填契约；补齐键盘分类切换、Escape 关闭、空结果及 1440/768/390 三档浏览器验收。
...
- 非秘密连接配置原子保存到本机 `RESEARCH_DATA_HOME`，密码仅进入系统凭据库；设置页支持保存、保留密码、移除和前往检测，密码不回填。新增“因子库研究”内置 Skill。
- MySQL 强制 TLS 但 `required_no_verify` 不验证证书，所有来源详情与快照明确记录风险。加入 PyMySQL 与 keyring 依赖；真实 MySQL 及非 macOS 的 Web 凭据库后端仍需轮换凭据和对应操作系统 CI 验证。桌面 sidecar 与安装包不在本次范围内。
...
- **Research Web 文件交付检查**：新增显式输出格式与 Skill 默认值、绑定幂等收据的哈希快照、单会话串行归属和严格沙箱内 DOCX/XLSX/HTML/Markdown/PNG 实际解析。详情单列交付状态、缺失/损坏原因，执行结束不等于文件交付完成；重命名和原生问题改用应用内表单。详见 `docs/research-web-delivery.md`；不代表真实模型完整产品验收。
  ...
  - 交付结果发布前以安全描述符复核已解析文件的大小及哈希，阻止解析后被修改或删除的文件误记为有效交付。
...
- **通用研究中心与机构级 Research Run（首个闭环）**：新增 PostgreSQL 权威 `ResearchRun` 运行时及 013/014 迁移，保存规范化 `ResearchSubject`、任务、证据输入、不可变版本化 Artifact、当前 Claim/Gate 投影和恢复信息。首版 LangGraph 只作为无事实持久化的执行层，完成来源规划、证据归一、Research Notes、叙事/财报综合、反方检验、质量门禁及发布投影；任何引用、数值、模板证据覆盖或冲突门禁失败均进入 `blocked`，补证后可恢复。
  ...
  - “资产观察”新增“启动深度研究”，通过共享预填契约携带股票/ETF/指数/主题上下文；每个领域不再增加独立导航页面。
  - 新增 `.agents/skills/a-share-deep-research/`，固化三表勾稽、舞弊红旗、叙事—估值检验、行业约束与来源降级方法；不复用占位 DOCX 生成器。
...
- **系统配置编辑器**：账号与 Key 池改为带标题的可访问集合，调度、连接与搜索参数保留为同一编辑表单的明确分组；高级配置按运行时、LLM 与文本分块导航。该界面调整保留秘密不回填、当前数据库安全摘要与环境锁定边界。

- **系统中心共享内容轨道**：系统配置页不再使用居中的窄容器，改为与“系统中心”共享完整内容宽度和左缘；两个页内导航在切换前后保持相同位置。

- **系统中心导航文案**：页内“资源与异常”更名为“系统中心”，并移除系统配置页的“控制台”冗余提示；底层导航与资源监控行为不变。
