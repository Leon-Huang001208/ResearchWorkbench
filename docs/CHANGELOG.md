...
- **整机容量告警并发安全**：整机 CPU/可用内存容量告警在连续三次有效压力或恢复样本后变更；无效读数中断连续计数。每个压力周期通过确定性主键和 savepoint 冲突恢复至多创建一条未解决事件，已解决事件保留为历史；warning 升级 critical 使用仅匹配未解决状态的原子详情更新，绝不覆盖用户的确认或解决状态。
