...
- Windows Runtime Profile 后置校验将 DSH 创建的 junction 与 POSIX symlink 同等视为候选，但仍要求
  严格目标位于固定源码；服务 PID 存活检查在 Windows 改用 CIM，避免 `os.kill(pid, 0)` 的
  `WinError 87` 掩盖真实启动错误。
