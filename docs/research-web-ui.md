...
页面使用 hash 路由：`#/fingpt`、`#/claw`、`#/workbench`、`#/workbench/assets`、`#/frameworks`、`#/frameworks/gold`、`#/frameworks/dollar`、`#/history`、`#/skills`、`#/operations` 与设置子路由。黄金和美元流动性详情各是一张带七个语义锚点的连续画布；旧 `?tab=` 地址继续定位相应锚点。框架间切换时，renderer 只接受与目标 slug 一致的快照；旧快照被丢弃并显示目标框架加载态。Dollar 的 SOFR−IORB 图显示最近 30 期，完整 60 期仍保留在快照用于研究状态。能力中心的规范入口使用 `#/skills?kind=skill|tool|workflow|data`，并以 `view=library|mine|plans|connections|market` 表示类型内二级视图；`market` 仅对 Tool 有效，规范地址为 `#/skills?kind=tool&view=market`，无 kind 的 market 也归一到 Tool。无 kind 的旧 plans/connections 链接分别归一到 Workflow/Tool。设置的规范地址是 `#/settings/general`、`#/settings/model`、`#/settings/data`、`#/settings/local`、`#/settings/docs`；`#/settings` 和非法子页回退至通用，旧 `#/settings?connection=<id>` 链接继续按来源进入数据源或本机集成。资产观察是主导航中的独立入口；研究台其余页面支持 `#/workbench/<section>` 与 `#/workbench?section=<section>`。会话地址形如 `#/fingpt?session=<encoded-id>`，刷新页面会重新读取该会话。
...
Phase 2A/2B/2C 通过 CI 后，MCP 市场、MCP Runtime 与 Automation 默认可见；部署可分别用三个 `RESEARCH_*_ENABLED=0` 环境变量关闭。该默认值变化不改变四类能力主导航、深链、弹窗或草稿不自动发送契约。
...
| `mcp-marketplace.mjs` | Tool 的只读 Registry 市场与受控 Runtime 管理：stale/offline、完整安装确认、持久安装恢复、探测/启停/移除、OAuth、风险分级、会话授权和一次性人工审批；外部 publisher 仍只生成命令 |
...
| `frameworks.mjs` / `frameworks/` | Framework Hub、Gold/Dollar 专属连续画布、11 张非重复用途的 Lieflat 图和绑定 slug/章节/缺口/snapshot revision 的解释/深度验证 Bot |
...
- Tool 的“MCP 市场”读取 `/mcp/registries`、`/mcp/servers` 与版本详情。API 返回有界 Unicode plain text，UI 只在最终 HTML sink 做一次转义，不渲染 Registry HTML、不加载远程图标。包状态分别读取 `package_type_supported` 和 `immutable_reference`；只有完整预览确认、安装和探测链路才产生真实 Runtime 状态。安装详情原样逐项展示 argv、来源、固定版本、全部哈希和环境变量名称，秘密值提交后立即清空且不进入页面状态或日志。
- 已安装记录从 `/mcp/installations` 恢复，不因刷新退回“未安装”。探测、启用、停用、移除、OAuth、工具分级与会话授权分别调用独立接口；启用不自动授权，策略或 schema 变化会暂停旧授权。高风险调用在 MCP 市场显示不含参数正文的一次性批准/拒绝队列。Runtime 开关关闭或读取失败时，Registry 浏览与离线缓存仍保持可用，所有安装入口禁用并说明原因。
...
- 设置在产品主导航内增加独立分类导航，拆为通用、模型服务、数据源、本机集成和架构文档；每次只渲染当前子页。DSH 模型配置不再折叠，DataHub 远程来源和本机能力互不混入。数据源页以统一协调器的“可用、检测中、待你处理、系统故障、尚未交付”概览、专业/API/公开分类、跨分类搜索和状态筛选组织 21 个远程来源；详情按登记、授权、探测、适配、可调用五阶段展示。本机集成页仍按本机服务、文件夹、Office 与金融插件、浏览器、本地 MCP 逐项展示事实，并使用同一五类责任汇总。分类导航使用不修改 Hash 路由的按钮，窄屏诊断行纵向排列且交互目标不低于 44px。报告自动化位于检查表之外并链接 Workflow 目录。
...
- Excel 应用与自动化桥、Wind 终端与 Excel 插件都在本机页分别展示，发现事实不能覆盖授权、真实验证或可调用结论。Wind 登录未验证时不显示“无需授权”；iFinD 专业终端在 macOS 标记不适用，普通同花顺客户端不作为其证据，SDK 与 HTTP API 继续由数据源页管理。报告自动化位于环境检查之外。数据源和本机页顶部“重新检测全部”调用统一异步批次 API；原单项探测与 Office 真实验证接口继续保留。旧 `.env` 迁移仍只属于数据源页：先显示不含值的预览，用户选择目标并二次确认后才调用迁移接口。
...
拖放/粘贴文件复用既有 multipart 流程。空首页不分配右侧空面板；研究详情中桌面也可显式收起/展开。活动/资料/文件标签仍只读取当前会话，Claw 会话/当前工作区切换继续复用安全下载与隔离预览。DataHub 的 15 个稳定工具常驻，Broker 按调用时最新状态选源；其他原生审批、停止、子 Agent 和交付语义不变。
...
“数据”主标签不是第四种运行器。页面从 `/data/catalog` 读取静态能力、来源和绑定，以“数据能力 / 数据源与连接”二级视图隔离展示，不与 Tool 卡片混排；保留分类、市场、状态和鉴权筛选。普通目录读取不联网；服务启动和设置页“重新检测全部”会按授权创建真实探测批次，来源详情仍可调用兼容的单项 probe。把能力放入研究草稿只选择相应的品牌无关 `datahub_*` Tool，不立即取数。
...
Workflow 的“运行计划”在 Phase 2C 展示通用 Automation、最近运行、下一次执行与旧报告日程兼容区。
新建任务必须显式选择锁定目标、时区、日程、输入和允许的 MCP 工具快照；投递渠道单独配置，秘密
字段提交后不回填。手动运行与重试都创建新的 Run，研究状态和投递状态分别显示。旧报告日程只有在
用户勾选并确认迁移后才停用，迁移失败保留原状态。
...
macOS 本机集成页为 Excel、Word、PowerPoint 和 Wind Excel 分别提供显式“验证”操作。页面轮询独立任务，展示进度、最近验证时间和安全化结论；离开该路由后旧轮询结果不会覆盖其他设置页。软件发现、用户授权、真实验证和当前可调用保持为四个独立事实。
...
当前 V0 只读取前端固定 fixture，并在页首明确声明演示数据不代表当前市场。Goldar 保留核心问题、研究链路、支持/拖累、下一项验证、数据覆盖与缺口；状态文案限定为偏强、中性、偏弱、待核验，不出现交易操作标签。七个章节及图表均由专属 renderer 输出，不建立跨领域大模板或第二份资产数据层。产品壳保留唯一 `main` 地标，Gold／Dollar 连续画布使用带名称的 `section`；900px 及以下将标题摘要上下排列，并把关闭的 Bot 入口停靠在横向锚点栏右端，不覆盖研究图表。完整契约见 [研究框架](architecture/research-web/08-research-frameworks.md)。

## 推理方法选择（2026-09-14）

能力中心增加独立“方法”页签，十个内置 Method 只读展示，不能新建、导入、复制或进入管理。
