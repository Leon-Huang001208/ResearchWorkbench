...
研究壳保留 AlphaFoundry 深蓝顶栏、深色窄主导航 rail、浅色二级会话栏及白色研究画布。rail 始终承载 FinGPT、Claw、能力中心、历史和设置；折叠仅作用于二级会话栏，不会用单一侧栏替代两层导航。≤1050px 时二级栏改为由 rail 打开的抽屉，仍保留新建研究、运行任务、最近会话及 Claw 工作区操作；即使该栏此前在桌面端折叠，抽屉打开时也不会保留 `aria-hidden`。顶栏检索只匹配已经加载的真实会话标题和 DSH 返回的 Skill 名称/简介；不会建立额外搜索 API 或展示演示能力。运行任务从完整的真实会话目录筛选，最近会话列表才限制为最近十项；hash 路由及已有 `data-*` 操作 selector 保持不变。
...
输入框继续使用既有附件、Skill、模型和格式契约。单独输入 `/` 或点击 `/ Skill` 会列出全部已启用的真实 Skill，继续输入再按名称/简介筛选；选择只写入草稿，不发送消息。拖放或粘贴文件复用既有 multipart 上传流程，必须先由原有控制器创建/定位会话。右侧改为“活动 / 资料 / 文件”标签；三个标签只读取当前会话详情。Claw 二级栏提供“会话 / 当前工作区”切换：会话恢复聊天主画布，当前工作区主画布复用现有 `renderDatasets`/`renderFiles` 展示该会话的资料、安全固定下载、文件下载与隔离预览；不增加存储、轮询或跨会话聚合，且切换不清除控制器草稿。运行参数默认折叠，失败活动展开并保留错误可见。
