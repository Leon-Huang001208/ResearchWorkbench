...
研究壳保留 AlphaFoundry 深蓝顶栏、窄产品导航、浅色会话侧栏及白色研究画布。顶栏检索只匹配已经加载的真实会话标题和 DSH 返回的 Skill 名称/简介；不会建立额外搜索 API 或展示演示能力。侧栏可折叠，运行任务和最近会话均来自现有会话目录。主导航为 FinGPT、Claw、能力中心、历史，设置位于底部；hash 路由及已有 `data-*` 操作 selector 保持不变。
...
输入框继续使用既有附件、Skill、模型和格式契约。`/` 打开已加载 Skill 的本地筛选；拖放或粘贴文件复用既有 multipart 上传流程，必须先由原有控制器创建/定位会话。右侧改为“活动 / 资料 / 文件”标签；三个标签只读取当前会话详情，Claw 工作区切换不会混入其他会话的资料或文件。运行参数默认折叠，失败活动展开并保留错误可见。
