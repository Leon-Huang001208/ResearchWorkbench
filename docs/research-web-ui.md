...
研究壳保留 AlphaFoundry 深蓝顶栏、深色窄主导航 rail、浅色二级会话栏及白色研究画布。rail 始终承载 FinGPT、Claw、能力中心、历史和设置；折叠仅作用于二级会话栏，不会用单一侧栏替代两层导航。≤1050px 时二级栏改为由 rail 打开的抽屉，仍保留新建研究、运行任务、最近会话及 Claw 工作区操作。顶栏检索只匹配已经加载的真实会话标题和 DSH 返回的 Skill 名称/简介；不会建立额外搜索 API 或展示演示能力。运行任务从完整的真实会话目录筛选，最近会话列表才限制为最近十项；hash 路由及已有 `data-*` 操作 selector 保持不变。

FinGPT 与 Claw 分别呈现首页。FinGPT 面向问题研究；Claw 明确目标、约束与预期交付。四个首页快捷入口严格筛选当前 DSH 目录中的 `document-reading`、`company-research`、`industry-research`、`fund-evaluation`；目录缺少其中任一项时不补造卡片。快捷入口和能力卡只会打开真实 Skill 描述或写入当前草稿的 `skill_id`，不会创建会话、提交消息或启动模型。

输入框继续使用既有附件、Skill、模型和格式契约。单独输入 `/` 或点击 `/ Skill` 会列出全部已启用的真实 Skill，继续输入再按名称/简介筛选；选择只写入草稿，不发送消息。拖放或粘贴文件复用既有 multipart 上传流程，必须先由原有控制器创建/定位会话。右侧改为“活动 / 资料 / 文件”标签；三个标签只读取当前会话详情。Claw 二级栏提供“会话 / 当前工作区”切换，工作区仅投影当前会话 `detail.datasets` 与 `detail.files`，不增加存储、轮询或跨会话聚合。运行参数默认折叠，失败活动展开并保留错误可见。
...
| Claw 工作区 | 会话/当前工作区切换只投影当前 `detail` 的资料与文件 | `research_web_ui_layout.test.mjs` |
