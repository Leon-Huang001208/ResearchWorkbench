...
- 设置在产品主导航内增加独立分类导航，拆为通用、模型服务、数据源、本机集成和架构文档；每次只渲染当前子页。DSH 模型配置不再折叠，DataHub 远程来源和本机能力互不混入。数据源页以真实连接状态概览、专业/API/公开分类、跨分类搜索和状态筛选组织 21 个远程来源，只展开当前分类；选择来源后在右侧详情面板复用原配置与探测表单。本机集成页采用“总体结论、环境检查、接入详情、能力目录”的单列诊断顺序；仅在存在多个本机能力组时显示紧凑选择器。桌面分类栏粘滞于内容侧，760px 以下改为可横向滚动且不低于 44px 的标签。
...
- Excel 是本机集成诊断，不是账号来源。本机页的整体结论只由 `callable`、接入完成状态和探测状态决定；Excel 自动化、Wind 插件、iFinD 插件和报告工作流以可比较列表展示当前服务设备的环境证据，不得用单项组件状态覆盖整体可调用结论。组件说明优先使用后端 `message/detail`，缺失时显示不含内部标识的状态解释；原四阶段状态后置在原生可展开的“接入详情”中。检测按钮沿用既有异步探测接口，在轮询完成、失败或超时前保持禁用；终态后重新加载连接目录。旧 `.env` 迁移仍只属于数据源页：先显示不含值的预览，用户选择目标并二次确认后才调用迁移接口。
