...
资源监控主机容量事件在 `metadata.dedupe_key` 记录稳定来源；同一键的每个已解决周期使用新的确定性 `alert_id`。未解决事件的升级仅条件更新 severity、标题、描述、阈值和 metadata，绝不改写 `status`、`acknowledged_at` 或 `resolved_at`。
