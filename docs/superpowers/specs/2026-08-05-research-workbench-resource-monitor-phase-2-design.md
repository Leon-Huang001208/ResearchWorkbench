# Research Workbench 资源监控第二期设计

## 目标

在既有实时资源看板上补足两项能力：

1. 将资源使用归因到 Research Workbench 管理的 Worker、任务类型和数据源；
2. 将异常和错误作为可确认、可恢复、可追溯的持久化事件展示在监控页，而不是只保留短期图表数据。

监控范围始终只限 Research Workbench。系统不得枚举、展示或持久化其他应用的进程信息。
...
`ResourceMonitoringService` 将保留 API 根进程及递归子进程采集，并额外接入**明确登记**的 Research Workbench Worker PID：
...
- 仅通过 API 根进程、既有 Research Workbench PID 文件和本功能专用目录发现进程；不调用系统全量进程枚举。
