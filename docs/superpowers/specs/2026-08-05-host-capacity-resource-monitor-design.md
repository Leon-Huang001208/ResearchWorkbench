...
- AlphaFoundry 原始快照继续在页面请求时每约 2 秒刷新，并在页面保留最近 5 分钟，用于定位短时任务突发。
...
3. 当前快照接口返回 `summary`（AlphaFoundry）与 `host`（整机）以及稳定的可用性/警告字段；前端继续每约 2 秒请求它用于短时显示。
...
5. `ResourceMonitorAlertService` 由常驻采集器在同一运行状态下连续调用，分别评估整机容量压力和 AlphaFoundry 进程压力，并继续使用既有告警/事件生命周期。
