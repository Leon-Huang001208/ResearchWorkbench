...
The in-app `notification` table remains authoritative. The Tauri shell registers the official notification plugin, enables the Tauri v2 global API for the existing no-bundler frontend, and grants only `is-permission-granted`, `request-permission`, and `notify`. Because the packaged bootstrap navigates the webview to the local FastAPI origin, a separate `notification-remote` capability grants exactly those three notification operations to `http://127.0.0.1:8765/*`; the default capability has no remote scope, so dialog, process, shell, and sidecar permissions remain local-only. The remote capability does not grant `localhost`, arbitrary hosts, or wildcard ports. There is no Rust command that accepts arbitrary title/body input. The frontend installs one single-flight durable poller at a 60-second interval. Each consumer must atomically claim a persisted record with `pending → desktop_delivering` before passing its API-provided title/body to `window.__TAURI__.notification.sendNotification`; a 409 claim conflict is skipped. Completion uses the expected transition `desktop_delivering → desktop_delivered|desktop_failed`, while permission denial/error conditionally updates from `pending`. Web non-Tauri sessions do nothing; permission refusal or native delivery failure never removes the inbox record or rolls back the alert event. DomainEvent/SSE invalidation can reduce latency in a later task without replacing the durable polling fallback.
