...
The in-app `notification` table remains authoritative. The Tauri shell registers the official notification plugin, enables the Tauri v2 global API for the existing no-bundler frontend, and grants only `is-permission-granted`, `request-permission`, and `notify`. Because the packaged bootstrap navigates the webview to the local FastAPI origin, the capability's remote scope is exactly `http://127.0.0.1:8765/*`; it does not grant `localhost`, arbitrary hosts, or wildcard ports. There is no Rust command that accepts arbitrary title/body input. The frontend reads pending records from the local FastAPI notification endpoint, passes only each persisted title/body to `window.__TAURI__.notification.sendNotification`, and records `desktop_delivered`, `desktop_permission_denied`, or `desktop_failed` through the delivery endpoint. Web non-Tauri sessions do nothing; permission refusal or native delivery failure never removes the inbox record or rolls back the alert event.
