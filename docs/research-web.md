...
- `local_integrations/verifiers.py`：用户显式触发的 macOS Office/Wind 真实验证；在独立进程组和受管验证目录中执行，超时会终止进程树，发现探测仍保持无副作用。Excel 要求全量重算并重开读值；Wind 对当前发布的 `huaan-etf-weekly` 受管副本依次执行烟测与全部策略刷新，并核对发布源哈希。成功结果才写入可调用状态，授权、登录、超时和公式失败均保持独立状态。
