...
先判断任务是否需要立即交互、是否会与活跃改动冲突，以及是否已经具备稳定、明确的验收标准。选择最小但足够的方式，不因任务看似复杂而跳过验证。

| 路由 | 适用条件 | 交付证据 | 不适用场景 |
|---|---|---|---|
| 本地快环 | 变更窄、需立即判断、不与活跃修改冲突 | 目标命令或浏览器结果 + 简洁 diff | 长任务、并发公共区域、或需要探索替代方案 |
| 独立 worktree | 并发、风险、多文件或需干净比较分支 | 路径、分支、针对验证、明确集成计划 | 只读或一行确认 |
| 后台/远程 | 验收稳定且确定但耗时，如仓库探索、CI 日志分析、审查或有界实现 | 任务范围、diff/报告、命令、未验证约束 | 交互秘密、破坏操作、发版决定、持续设计判断 |

选择后仍应先阅读相关规则、目标文件及其直接依赖；任务范围、所有权或验收不明确时，先同步并拆分，而不是扩大执行范围。

## 桌面端例外（Desktop exception）

worktree 只隔离文件冲突，绝不取代桌面端原生 Windows CI，也不能取代真实 Windows 安装级烟测。凡影响 `src-tauri/`、`desktop/`、`scripts/desktop/`、sidecar、桌面配置或路径、安装包、自动更新、Excel/Wind 集成的改动，都必须遵循 [`desktop_packaging.md`](desktop_packaging.md) 的跨平台验收要求。

交付时应明确：本地验证只说明本机结果；原生 Windows CI 必须覆盖依赖安装、Python sidecar 构建、Tauri Windows 安装包构建及基础启动/`/health` 检查；发布前必须在真实 Windows 环境完成安装级烟测。macOS 本地运行或隔离 worktree 都不能证明 Windows 可用。
