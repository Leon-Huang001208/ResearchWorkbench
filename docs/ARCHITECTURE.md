...
诊断与手动刷新继续由设置、运行与用量及对应业务页面承担。设置在同一产品壳内使用五个互斥的 Hash 子页，仅按当前子页加载所需状态。进程内 `IntegrationCoordinator` 聚合 DataHub、本机诊断和 Tabbit，启动恢复安全快照后异步探测，并通过 `/api/research/integrations/*` 提供全量批次；原数据源与本机接口继续兼容。
...
数据源子页在前端将 21 个远程来源投影为专业、API、公开三类工作台，并消费统一五阶段状态派生汇总、搜索和筛选结果。卡片选择仅控制同页配置抽屉，保存、移除和凭据处理仍走原有连接中心接口；单项探测保持兼容，全量重检由协调器批次执行。本机集成仍位于独立设置子页，协调器只是 Host 内模块，不引入新守护进程。
...
研究框架位于同一 Research Web Host 内：薄注册表与单一调度器管理 Gold v2、Dollar v1 的生命周期和原子快照，领域定义、评分、采集、上下文及 renderer 保持独立。`#/frameworks/gold` 与 `#/frameworks/dollar` 都是七锚点连续画布；路由切换时前端先核对快照 slug，不允许旧框架快照进入目标 renderer。Dollar renderer 只把 SOFR−IORB 完整快照的最近 30 期交给 F2 图，评分和证据仍使用完整 60 期窗口。页面 Bot 复用唯一 DSH 执行链；默认解释预设无工具，显式深度验证另建只读检索会话并绑定 slug、方法版本与精确快照 revision。该模块不增加第二个研究引擎、资产终端或事实数据库。
