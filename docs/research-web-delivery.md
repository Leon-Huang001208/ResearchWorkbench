...
沙箱返回后、生成最终交付状态并保存收据之前，宿主再通过 `Store.open_file` 的
安全描述符复核所有已通过候选文件的字节数与 SHA-256。此步骤仅哈希，不解析
文件；变化、消失或安全读取失败均使该文件无效，并重新计算缺失格式。该检查是
发布时点验证，不将会话 outputs 目录变成不可变归档，也不保证发布后外部修改仍有效。
