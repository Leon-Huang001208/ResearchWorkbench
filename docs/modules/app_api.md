...
- `POST /api/config/{section}/test` merges unsaved form values with retained secrets without writing the file or mutating `os.environ`. LLM, ZhiQiu, and iFinD use real short-timeout probes; database performs URL structure validation; advanced does not support testing. ZhiQiu and iFinD use ordered account pools; iFinD persists `IFIND_ACCOUNTS_JSON` and mirrors the first account to legacy runtime fields.
