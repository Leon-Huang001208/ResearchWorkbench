...
`services` contains Research Workbench's business orchestration logic.
...
- `ResourceTaskRegistry` 为 Research Workbench 的抓取、PDF、知识处理、Wind 和报告任务记录受控运行上下文，并以每 PID 原子 JSON 快照供 API 进程读取。
...
- `ResourceMonitorAlertService` 将任务失败、受控进程缺失、采样失败和持续资源压力以 `source_scope=research_workbench` 保存为既有 Monitoring 告警/事件状态机中的 `resource_monitoring` 事件；另以 `source_scope=host_capacity` 评估整机 CPU（85%/95%）和可用内存（15%/8%）容量压力。所有资源事件通过仓储的确定性周期 ID 和 savepoint 冲突恢复，确保并发周期至多一个未解决事件；两类主机容量均需连续三个有限且处于 0–100 的样本触发或恢复，无效字段会中断该指标的连续计数。warning 升为 critical 时调用条件更新，仅更新详情且保留确认/解决状态。
...
- 运行时只在持久化就绪且非 `RESEARCH_PREVIEW=1` 时启动；关闭 API 时先以可配置的有限超时停止资源采样线程，再停止其他调度器。超时仅记录 `RuntimeStopTimeout` warning，不阻断关闭。
...
- CLI `rwb ask` / API `POST /api/llm/ask`（直接调 AskService）。
