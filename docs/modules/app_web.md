...
- 资源监控只呈现 API 根进程及递归子进程的聚合 CPU、内存、磁盘读/写速率、线程数和连接数；“连接数”是进程连接计数，不是每进程网络字节数。图表依赖可选的 ECharts，图表组件未就绪时数字摘要和进程表仍会更新；API 的降级状态会保留上一帧并显示不可用提示。
- 进程表支持按 CPU、内存或磁盘 I/O 请求排序，行可打开详情抽屉；命令摘要、状态和数值均用 DOM `textContent` 填充，避免把进程元数据作为 HTML 插入。不同操作系统或进程权限下，psutil 可能不能提供 I/O 或连接数字，界面显示 `--` 和 API 的公开降级状态，而不展示底层异常。
...
- 2026-08-05: 新增“系统监控”工作区页面，展示限定 API 进程树的资源摘要、CPU/内存历史图、可排序进程表和详情抽屉；页面仅在可见且激活时轮询，离开后中止请求，图表缺失或字段受限时保持可读降级。
