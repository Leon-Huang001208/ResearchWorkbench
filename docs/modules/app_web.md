...
- 配置卡片会打开对应的编辑弹窗。桌面宽度下，账号与 Key 采用紧凑表格展示，便于快速浏览和编辑；窄屏下同一内容改为带字段标签的纵向堆叠行，避免表格横向溢出。尚未添加账号或 Key 时，空状态会在列表内直接给出下一步操作入口；支持测试的配置会说明“测试连接不会保存当前更改”，主要操作为“保存更改”。
- 高级配置使用分类导航，并将参数按类别组织为分组列表；该布局仅改变编辑与浏览方式，不改变字段含义或保存范围。
- 由当前启动配置管理的字段仍会保持只读，并在受影响字段、行或集合附近给出说明；秘密值不会回填到界面。Provider、任务路由和账号/Key 池按原子集合锁定：任一受管项会使该集合只读且不随保存提交，未受管的同分区独立设置仍可保存。账号/Key 表格、窄屏堆叠行、空状态操作和高级配置分组均不影响这些启动环境锁定或秘密处理规则。
...
- 2026-07-27: 重整系统配置编辑弹窗：桌面端账号/Key 使用紧凑表格，窄屏改为带标签的纵向堆叠行；空列表内提供下一步添加操作。高级配置采用分类导航和按类别分组的参数列表。该调整不改变启动环境锁定、原子集合保存边界或秘密值不回填的安全规则。
