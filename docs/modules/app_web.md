...
- 侧栏以唯一“系统”入口承载“资源与异常”（默认）和“系统配置”两个页内标签。`app.js` 将旧的 `resource-monitor`/`config` 本地导航值迁移为 `system` 加受限子标签；资源监控仅在“资源与异常”可见且文档可见时采样，离开该标签或切到后台会取消定时器、在途请求并释放图表。它先读 300 秒 AlphaFoundry 历史，再按成功 2 秒、连续失败 4/6/8/10 秒退避读取当前快照，页面内至多保留 150 个点；另在首次激活、恢复可见和每 60 秒读取一次 `host-history` 的 24 小时整机容量历史，失败时保留上一份成功曲线。后一条路径只读取历史；持续的分钟采集由非预览 API 运行时负责。
...
- 2026-08-07: 将“系统监控”与“系统配置”收拢为单一“系统”入口，采用页内“资源与异常 / 系统配置”标签并迁移旧本地导航；资源页取消常态 `ok` 徽标，以异常或采样不可用提示替代，异常历史改为可键盘操作的深色单选浮层，置顶事件采用紧凑行。
