...
- `knowledge_layer/retrieval/vector_store.py` honors `RESEARCH_DISABLE_LOCAL_EMBEDDINGS=1`; tests set this by default. Runtime local embeddings use `RESEARCH_LOCAL_EMBEDDING_MODEL_PATH` first, otherwise sentence-transformers is called with `local_files_only=True`; set `RESEARCH_ALLOW_EMBEDDING_DOWNLOAD=1` only when first-time Hugging Face download is intentional.
