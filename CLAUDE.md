...
- `af-auto-000` - 初始仓库审计和验证 ✅ 已完成
- `af-auto-001` - 测试修复和覆盖提升 ✅ 已完成
- `af-auto-002` - 数据摄入和报告模板 ✅ 已完成
- `af-auto-003` - UI 设计规范落地和 Playwright 调试 ✅ 已完成
...
读取对应的任务文件 `.ai/tasks/task_<任务集ID>.json` 并选择一个任务（优先级）：
...
1. **首先创建/更新任务集专用进度文件**：`.ai/progress/progress_af_auto_xxx.md`
   - 记录该任务集的完整详细进度
   - 包含所有任务的完成情况、报告位置、关键发现等
2. **然后更新通用进度文件**：`.ai/progress/progress.md`
   - 添加该任务集的摘要条目到总索引表格
   - 添加任务集摘要说明

**示例**：
- 任务集 `af-auto-003` 对应 `progress_af_auto_003.md`
- 所有任务集在 `progress.md` 中有一个索引条目
...
5. **每个完成任务必须更新对应的 task_af_auto_xxx.json 和 progress_af_auto_xxx.md，以及在 progress.md 中更新摘要**
