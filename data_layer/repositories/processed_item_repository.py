...
def item_exists(db: Session, item_id: str) -> bool:
    """检查项目是否已处理"""
    return db.query(ProcessedItemV1DB).filter(
        ProcessedItemV1DB.item_id == item_id
    ).first() is not None
...
def item_exists_by_hash(db: Session, content_hash: str) -> bool:
    ...
    if not content_hash:
        return False
    return db.query(ProcessedItemV1DB).filter(
        ProcessedItemV1DB.content_hash == content_hash
    ).first() is not None


def add_processed_item(db: Session, item: ProcessedItemV1DB) -> ProcessedItemV1DB:
    """添加已处理项目"""
    existing = db.query(ProcessedItemV1DB).filter(
        ProcessedItemV1DB.item_id == item.item_id
    ).first()
...
def get_recent_processed(
    ...
    """获取最近处理的项目"""
    query = db.query(ProcessedItemV1DB).filter(
    ...
    return query.order_by(
        ProcessedItemV1DB.first_seen_at.desc()
    ).limit(limit).all()
...
def get_processed_stats(
    ...
    if not source_type:
        type_stats = db.query(
            ProcessedItemV1DB.source_type,
            func.count(ProcessedItemV1DB.item_id)
        ).group_by(ProcessedItemV1DB.source_type).all()
...
def get_processed_stats_by_day(
    db: Session,
    source_type: str,
    days: int = 7
) -> list[dict]:
    ...
    items = db.query(ProcessedItemV1DB).filter(
        and_(
            ProcessedItemV1DB.source_type == source_type,
