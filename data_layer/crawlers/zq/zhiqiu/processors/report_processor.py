        ...
        **kwargs
        ...
        if isinstance(reports_list, dict):
            ...
            return self._process_old_format(
                output_file, prompt, enable_core, enable_viewpoint, enable_companies,
                enable_pdf, os.path.join(output_dir, pdf_dir), ai_interval, output_dir,
                state_manager, skip_existing
            )
        else:
            return self._process_new_format(
                data, output_file, prompt, enable_core, enable_viewpoint, enable_companies,
                enable_pdf, os.path.join(output_dir, pdf_dir), ai_interval, output_dir,
                state_manager, skip_existing
            )
    ...
    def _process_new_format(
        ...
        for report in reports_list:
            ...
            item = self._build_item_from_report(
                report, prompt, enable_core, enable_viewpoint, enable_companies,
                enable_pdf, pdf_root_dir, ai_interval
            )
            if item:
                results.append(item)
                new_reports.append(item)
