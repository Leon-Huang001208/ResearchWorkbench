...
class MeetingFetcher(BaseFetcher):
    ...
    def __init__(self, config: Optional[MeetingConfig] = None):
        ...
        if self.config.state_path:
            try:
                self._state_manager = MeetingStateManager(self.config.state_path, self.config.verbose)
...
"""
    ...
    parser.add_argument('--skip-existing', action='store_true', default=True, help='跳过已存在的纪要 (默认: True)')
