...
class TALibProvider:
    ...
    def calculate(self, df: pd.DataFrame, symbol: str) -> TechnicalIndicators:
        ...
        # 获取最新日期
        latest_date = df.index[-1].strftime('%Y-%m-%d') if hasattr(df.index, 'strftime') else str(df.index[-1])
        ...
        try:
            if len(close_data) >= 26 + 9:
                macd, macd_signal, macd_hist = talib.MACD(close_data, fastperiod=12, slowperiod=26, signalperiod=9)
                result.macd = {
        ...
        try:
            if len(close_data) >= 20:
                upper, middle, lower = talib.BBANDS(close_data, timeperiod=20, nbdevup=2, nbdevdn=2, matype=0)
                result.boll = {
