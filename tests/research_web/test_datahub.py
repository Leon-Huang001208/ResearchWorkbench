...
def query(module, **kw):
    return module.Query(
        source="fund_nav", code="000001", start_date="2025-01-01", end_date="2025-12-31", **kw
    )
...
async def test_actual_twenty_row_pages_are_drained_and_snapshot_is_scoped(hub_module, tmp_path):
    ...
    assert (result["row_count"], result["pages_fetched"], result["provider_total"]) == (43, 3, 43)
    ...
    assert detail["actual_range"] == {"start_date": "2025-11-19", "end_date": "2025-12-31"}
    ...
    def respond(request):
        ...
        if calls == 1:
            return httpx.Response(
                200, json=nav_page([nav_row("2025-12-31"), nav_row("2025-12-30")], 4, size=2)
            )
        if problem == "http":
...
async def test_real_holdings_headers_with_breaks_and_profile_asset_label(hub_module, tmp_path):
    ...
    result = await hub.query(
        sid, "real-header", hub_module.Query(source="fund_holdings", code="000001", year=2025)
    )
    assert result["status"] == "snapshot"
...
def test_launcher_prepares_private_bridge_config_without_secret_environment(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        launch_runtime.subprocess, "check_output", lambda *args, **kw: launch_runtime.PINNED_COMMIT
    )
    data = tmp_path / "data"
