...
async function fixture() {
  ...
  return {root,sid,cwd,control,tool:tools.get('datahub_get_fund_data'),tools,ctx,exec,setOutcome(value){outcome=value;}};
}
...
test('denial cancellation unsafe parameters and untrusted ancestry send zero HTTP',async()=>{
  ...
  try{
    ...
    for(const outcome of ['rejected','cancelled','unavailable']){f.setOutcome(outcome);await assert.rejects(f.tool.execute(valid,f.exec),/HTTP not sent/);}
