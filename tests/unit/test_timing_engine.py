...
class TestTimingEngineService:
    ...
    def test_full_workflow_proceed(self):
        ...
        timing = service.calculate_timing_fit(
            regime=0.8, flow=0.8, theme_diffusion=0.8, crowding=0.8
        )
        ...
        readiness = service.calculate_readiness(
            thesis_quality=0.8, historical_metrics=historical, timing_factors=timing
        )
    ...
    def test_full_workflow_block(self):
        ...
        service = TimingEngineService()
        timing = service.calculate_timing_fit(
            regime=0.2, flow=0.2, theme_diffusion=0.3, crowding=0.2
        )
        historical = service.calculate_historical_edge(
            event_count=3, average_excess_return=0.01, win_rate=0.3, max_drawdown_after_entry=-0.2
        )
        readiness = service.calculate_readiness(
            thesis_quality=0.4, historical_metrics=historical, timing_factors=timing
        )
