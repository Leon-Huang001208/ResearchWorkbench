...
from scripts.rebuild_derived_state import RebuildReporter
...
def test_reporter_add_outcome_results():
    ...
    reporter.add_outcome_result(
        "signal3", rebuilt=False, missing_market=True, reason="No price data"
    )
...
def test_reporter_print_summary():
    ...
    reporter.add_signal_result("event2", rebuilt=False)
    reporter.add_outcome_result(
        "signal3", rebuilt=False, missing_market=True, reason="No market data"
    )
