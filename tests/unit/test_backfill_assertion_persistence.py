...
def test_assertion_persistence_after_backfill(temp_db, assertion_extractor, event_ingestor):
    ...
    try:
        ...
        changed, source_doc = upsert_source_document(
            temp_db, temp_path, loaded, force_reextract=False
        )
...
def test_backfill_idempotency(temp_db, assertion_extractor, event_ingestor):
    ...
    try:
        ...
        # First run
        changed1, source_doc = upsert_source_document(
            temp_db, temp_path, loaded, force_reextract=False
        )
        temp_db.commit()
        ...
        count_after_first = (
            temp_db.query(Assertion).filter_by(source_doc_id=source_doc.doc_id).count()
        )

        # Second run - should skip existing assertions
        changed2, source_doc2 = upsert_source_document(
            temp_db, temp_path, loaded, force_reextract=False
        )
        ...
        assert (
            temp_db.query(Assertion).count() == count_after_first
        )  # still same count, just updated
