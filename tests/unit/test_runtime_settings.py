...
def test_desktop_runtime_honors_data_directory_from_explicit_environment(tmp_path):
    from core.settings.runtime import resolve_runtime_context

    data_dir = tmp_path / "controlled-data"
    context = resolve_runtime_context(
        environ={
            "ALPHAFOUNDRY_RUN_MODE": "desktop",
            "ALPHAFOUNDRY_DESKTOP_DATA_DIR": str(data_dir),
        },
        project_root=tmp_path / "project",
    )

    assert context.data_dir == data_dir
    assert context.env_path == data_dir / ".env"


def test_explicit_config_file_and_backend_url_override_mode_defaults(tmp_path):
    from core.settings.runtime import resolve_runtime_context
