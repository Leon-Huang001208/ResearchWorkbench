...
@pytest.fixture
def huaan_preview_reports() -> dict[str, Path]:
    """Return versioned-path preview samples when local Huaan DOCX fixtures exist."""
    generated_dir = Path("report_projects/华安ETF周报/generated")
    reports = {
        "anchored": generated_dir / "20260417_华安ETF周报.docx",
        "embedded": generated_dir / "20260515_华安ETF周报.docx",
    }
    if not all(path.is_file() for path in reports.values()):
        pytest.skip("requires unversioned Huaan generated DOCX preview fixtures")
    return reports
...
def test_word_preview_normalizes_anchored_chart_below_caption(
    huaan_preview_reports: dict[str, Path], tmp_path: Path
):
    """历史报告里的浮动图表应在预览副本中移到“图1”标题下方。"""
    import app.api.routes.report_projects as report_projects_route

    source = huaan_preview_reports["anchored"]
...
def test_word_preview_normalizes_chart_embedded_in_caption_paragraph(
    huaan_preview_reports: dict[str, Path], tmp_path: Path
):
    """标题段落自身携带浮动图时，也应拆成标题在前、图表在后。"""
    import app.api.routes.report_projects as report_projects_route

    source = huaan_preview_reports["embedded"]
    normalized = tmp_path / source.name
...
def test_build_word_pdf_preview_uses_normalized_copy_for_soffice(
    huaan_preview_reports: dict[str, Path], tmp_path: Path, monkeypatch
):
