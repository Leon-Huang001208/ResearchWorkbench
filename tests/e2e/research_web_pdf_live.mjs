...
try {
  ...
  const resume=process.env.ALPHAFOUNDRY_ACCEPTANCE_SESSION;
  if (resume) {
  ...
  await page.getByRole('textbox',{name:'研究问题',exact:true}).fill('用所选资料解读 Skill，实际读取刚上传的 dsh-report.pdf（国金证券 DSH 研报）。只用af_run_script与现有PDF库读取PDF物理第14和15页，即0-based索引13和14，不联网不安装不读取其他文件。分别概括两页主要内容，每条标注PDF物理页码；若印刷页码不同同时注明。给出每页一段不超过20字的原文锚点供核对。只答简短中文，不生成报告文件，不复述你没有实际读取的页；若页数不足明确失败。');
  ...
  assert.ok(detail.activities.some(a=>a.title==='af_run_script' && a.status==='completed'));
