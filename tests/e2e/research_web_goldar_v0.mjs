/** Read-only visual acceptance for the Goldar continuous research canvas. */
...
const outputRoot = path.join(projectRoot, 'outputs/goldar-v1');
...
const { goldFallbackData } = await import(pathToFileURL(path.join(uiRoot, 'frameworks.mjs')));
...
async function fulfill(route) {
  ...
  if (url.pathname.startsWith('/api/research/')) {
    const fallback = goldFallbackData();
    const payload = url.pathname === '/api/research/frameworks/gold/data'
      ? fallback
      : url.pathname === '/api/research/frameworks'
        ? { items: [{ ...fallback.framework, status: fallback.snapshot.status, coverage: fallback.snapshot.coverage, updated_at: fallback.snapshot.as_of }] }
...
try {
  for (const theme of ['light', 'dark']) {
    for (const [width, height] of viewports) {
      ...
      assert.equal(await page.locator('[data-lieflat-basics]').count(), 4);
      ...
      if (theme === 'light' && [1440, 390].includes(width)) {
        ...
        const filename = `light-${width}x${height}-bot.png`;
  ...
  await page.route('**/*', fulfill);
  await page.goto(`${origin}/#/frameworks/gold`, { waitUntil: 'networkidle' });
  const anchors = ['drivers', 'supply', 'cycle', 'positioning', 'allocation', 'evidence'];
  for (const tab of anchors) {
    ...
    assert.match(await page.locator('.framework-anchor-rail a[aria-current="location"]').textContent(), new RegExp(({ drivers: '定价驱动', supply: '供需与资金', cycle: '周期与宏观', positioning: '持仓与期权', allocation: '情景与配置', evidence: '事件与证据' })[tab]));
  ...
  report.checks.push({ reducedMotion: true, keyboardAnchors: anchors.length });
