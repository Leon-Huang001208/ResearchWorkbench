...
3. 复杂任务使用 DSH 原生 subagent 至少分业务竞争、财务风险两方向；各自仅写唯一前缀文件，主代理整合。不能虚构子任务已执行。
   需补充公开资讯时，父 Agent 使用 datahub_search_news 经用户批准准备资料；财务与公告能力完成来源适配后分别使用 datahub_get_financials、datahub_search_announcements。读取 manifest_json 及 inputs/datasets 的 manifest.json，核对 dataset_id、hash、status、实际期间、行数、missing 和 limitations，再分配两个子 Agent 读取同一 CSV/JSON；原生委派 approval=never，不重复取数、不重试被拒请求。仅使用已经取得的口径计算。
