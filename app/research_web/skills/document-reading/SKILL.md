...
4. 发现冲突或缺失时显式列出，不能用零补缺失。
   如需公开补充资料，先由父Agent经af_public_data的原生审批准备资料，读取manifest_json和inputs/datasets的manifest.json，核对dataset_id、hash、status、实际期间、行数、missing/limitations；复杂Claw任务再分配两个子Agent读取同一CSV/JSON，子Agent不重复取数。现有上传资料不必额外联网，所有计算限定已取得口径。
