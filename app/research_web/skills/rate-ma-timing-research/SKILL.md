...
输入必须严格符合 `references/input-schema.json`，日期严格递增且末日等于数据截止日；provider、字段、百分比/点位/小数单位、交易日语义和复权口径必须完全匹配。历史少于 `moving_average_window + 1` 返回 `insufficient_history`，未来日期、未知字段、非有限数值或不等价来源失败关闭。完整输出不超过 64 KiB，成功 stdout 无尾随换行。
