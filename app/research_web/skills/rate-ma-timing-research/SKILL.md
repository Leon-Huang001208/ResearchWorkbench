...
输入必须严格符合 `references/input-schema.json`，并提供资产与利率两条序列各自的 `role`、identity、version 和 tenor。角色分别固定为资产指数与政府债收益率，version 分别固定为 `close_v1` 与 `yield_pct_v1`，期限分别固定为 `spot` 与 `10Y`；identity 必须是非空、受限字符的业务标识。synthetic provider 还必须匹配提交 fixture 的精确 identity；`user_input` 可提供其他真实业务 identity 并在输出中原样回传，但不能交换角色、重复身份或改变版本/期限。Wind/DataHub 只有命中 `references/field-mapping.json` 的精确生产身份白名单时才可等价；当前白名单为空，因此均失败关闭。日期严格递增且末日等于数据截止日；provider、字段、百分比/点位/小数单位、交易日语义和复权口径必须完全匹配。历史少于 `moving_average_window + 1` 返回 `insufficient_history`，身份、版本、期限、未来日期、未知字段、非有限数值或不等价来源均失败关闭。完整输出不超过 64 KiB，成功 stdout 无尾随换行。
