...
日期严格使用 `YYYY-MM-DD`；市场快照、宽度、行业、主题和新闻集合均为必填，宽度计数只能是非负整数，币种固定为 CNY。`source_hashes` 存在时必须是对象，key 须为非空且首尾无空白的规范名称，value 须为 SHA-256；显式 null 或非规范 key 会被拒绝，字段缺失时结果降级为 partial 并披露 limitation。
