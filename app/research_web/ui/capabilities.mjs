...
const capabilityKinds = ['skill', 'tool', 'workflow', 'data'];

export function reportWorkflowEligibility(item) {
  const report = reportConfig(item);
  if (!report) return { report: null, eligible: Boolean(item?.enabled && item?.version), version: item?.version || null, reason: '' };
  const version = Number(report.package_version);
  const readiness = typeof report.readiness === 'string'
    ? report.readiness
...
export function capabilityTabKey(key, current) {
  if (!['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(key)) return { handled: false };
  const index = Math.max(0, capabilityKinds.indexOf(current));
  const next = key === 'Home' ? 0 : key === 'End' ? capabilityKinds.length - 1 : (index + (key === 'ArrowRight' ? 1 : -1) + capabilityKinds.length) % capabilityKinds.length;
  return { handled: true, kind: capabilityKinds[next] };
}
...
function capabilityCard(item, isTool, kind, busy) {
  ...
  const selectable = report ? eligibility.eligible : Boolean(item.enabled);
  const reportMeta = report ? `<div class="report-workflow-card-meta"><span>资源 ${list(report.resources).length} 项</span><span>${eligibility.eligible ? `锁定 v${e(eligibility.version)}` : '报告 Workflow 暂不可用'}</span><span>${e(report.schedule?.enabled ? report.schedule.label || '日程已启用' : '手动运行')}</span></div>${eligibility.reason ? `<p class="small muted" role="status">${e(eligibility.reason)}</p>` : ''}` : '';
  return `<article class="skill-card ${report ? 'report-workflow-card' : ''}"><div class="card-top"><span class="skill-icon">${icon(isTool ? 'grid' : kind === 'workflow' ? 'layers' : ({ 'document-reading': 'document', 'company-research': 'company', 'industry-research': 'industry', 'fund-evaluation': 'chart' })[item.id] || 'document')}</span><h2>${e(item.name)}</h2>${report ? '<span class="badge">报告 Workflow</span>' : ''}</div><p class="card-description">${e(item.description || '待补充简介')}</p>${reportMeta}<div class="card-bottom"><span>${isTool ? item.selectable ? '可选研究工具' : 'Agent 内部控制' : `${e(item.category)} · ${item.builtin ? '内置' : '我的'}${item.has_draft ? ' · 有未发布草稿' : ''}`}</span><span class="card-formats">${!isTool ? line(item.metadata?.default_formats, '无需文件') : ''}</span></div>${!isTool ? `<details class="card-requirements"><summary>输入要求与场景</summary><p class="small">场景：${line(item.metadata?.scenarios, '待补充')}</p><p class="small">输入：${e(list(item.metadata?.inputs).map(input => `${input.label}（${input.type}${input.required ? ' · 必填' : ' · 可选'}）`).join('、') || '待补充')}</p><p class="small">输出：${line(item.metadata?.default_formats, '无需文件')}</p></details>` : ''}<div class="card-state">${isTool ? '<span class="badge">只读 Tool</span>' : statusBadge(item)}<div class="button-row">${button('查看详情', `${isTool ? 'data-tool-detail' : 'data-skill-detail'}="${e(item.id)}"`, busy)}${isTool ? item.selectable ? button('放入草稿', `data-use-tool="${e(item.id)}"`, busy) : '' : button('放入草稿', `data-use-skill="${e(item.id)}"`, busy || !selectable)}</div></div></article>`;
}
