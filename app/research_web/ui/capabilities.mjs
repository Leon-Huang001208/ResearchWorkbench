...
export function renderCapabilityCatalog({ items = [], tools = [], dataCatalog = {}, dataView = 'capabilities', dataMarket = '', dataStatus = '', dataAuth = '', probe = null, kind = 'skill', source = 'all', category = '', query = '', busy = false, loading = false, error = '' } = {}) {
  if (kind === 'data') return `<header class="page-header capability-heading"><div><h1>能力中心</h1><p class="muted">统一查看数据能力、候选来源与真实接入状态。</p></div>${button('刷新目录', 'data-cap-refresh', busy)}</header><div class="capability-tabs" role="tablist" aria-label="能力类型">${[['skill', 'Skill'], ['tool', 'Tool'], ['workflow', 'Workflow'], ['data', '数据']].map(([value, label]) => `<button type="button" role="tab" aria-selected="${kind === value}" class="button ${kind === value ? 'primary' : ''}" data-cap-kind="${value}">${label}</button>`).join('')}</div>${renderDataCatalog({ catalog: dataCatalog, view: dataView, query, category, market: dataMarket, status: dataStatus, auth: dataAuth, busy, loading, error, probe })}`;
  ...
  return `<header class="page-header capability-heading"><div><h1>能力中心</h1><p class="muted">Skill、Workflow、真实工具与数据目录。选择只准备草稿；由你确认开始。</p></div><div class="catalog-actions">${actions}${button('刷新目录', 'data-cap-refresh', busy)}</div></header><div class="capability-tabs" role="tablist" aria-label="能力类型">${[['skill', 'Skill'], ['tool', 'Tool'], ['workflow', 'Workflow'], ['data', '数据']].map(([value, label]) => `<button type="button" role="tab" aria-selected="${kind === value}" class="button ${kind === value ? 'primary' : ''}" data-cap-kind="${value}">${label}</button>`).join('')}</div><div class="capability-filters"><label class="search-box"><span aria-hidden="true">⌕</span><input id="cap-search" data-cap-query type="search" aria-label="搜索能力名称和简介" value="${e(query)}" placeholder="搜索名称、简介或技术 ID"></label>${!isTool ? `<label>来源<select id="cap-source" data-cap-source>${[['all', '全部'], ['builtin', '内置'], ['mine', '我的']].map(([v, l]) => `<option value="${v}" ${source === v ? 'selected' : ''}>${l}</option>`).join('')}</select></label><label>分类<select id="cap-category" data-cap-category><option value="">全部分类</option>${categories.map(value => `<option value="${e(value)}" ${category === value ? 'selected' : ''}>${e(value)}</option>`).join('')}</select></label>` : ''}</div>${isTool ? '<p class="notice warning">只读目录声明，不代表实例在线、凭据齐备或已获审批。工具选择不改变任何原生权限。</p>' : ''}${error ? `<p class="notice error" role="alert">${e(error)}</p>` : ''}${loading ? '<p role="status">正在读取能力目录…</p>' : ''}${results.length ? `<div class="skills-grid capability-grid">${results.map(item => `<article class="skill-card"><div class="card-top"><span class="skill-icon">${icon(isTool ? 'grid' : kind === 'workflow' ? 'layers' : ({ 'document-reading': 'document', 'company-research': 'company', 'industry-research': 'industry', 'fund-evaluation': 'chart' })[item.id] || 'document')}</span><h2>${e(item.name)}</h2></div><p class="card-description">${e(item.description || '待补充简介')}</p><div class="card-bottom"><span>${isTool ? item.selectable ? '可选研究工具' : 'Agent 内部控制' : `${e(item.category)} · ${item.builtin ? '内置' : '我的'}${item.has_draft ? ' · 有未发布草稿' : ''}`}</span><span class="card-formats">${!isTool ? line(item.metadata?.default_formats, '无需文件') : ''}</span></div>${!isTool ? `<details class="card-requirements"><summary>输入要求与场景</summary><p class="small">场景：${line(item.metadata?.scenarios, '待补充')}</p><p class="small">输入：${e(list(item.metadata?.inputs).map(input => `${input.label}（${input.type}${input.required ? ' · 必填' : ' · 可选'}）`).join('、') || '待补充')}</p><p class="small">输出：${line(item.metadata?.default_formats, '无需文件')}</p></details>` : ''}<div class="card-state">${isTool ? '<span class="badge">只读 Tool</span>' : statusBadge(item)}<div class="button-row">${button('查看详情', `${isTool ? 'data-tool-detail' : 'data-skill-detail'}="${e(item.id)}"`, busy)}${isTool ? item.selectable ? button('放入草稿', `data-use-tool="${e(item.id)}"`, busy) : '' : button('放入草稿', `data-use-skill="${e(item.id)}"`, busy || !item.enabled)}</div></div></article>`).join('')}</div>` : empty('没有匹配的能力', '调整搜索、来源或分类；目录不会补充演示内容。')}`;
}
