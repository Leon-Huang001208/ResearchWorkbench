...
export function skillMatches(query, skills = []) {
  ...
  const enabled = (Array.isArray(skills) ? skills : []).filter((skill) => {
    if (skill?.kind === 'method') return false;
...
export function tabbitMentionQuery(value) {
...
export function tabbitKey(key, index, matches) {
  if (key === 'Escape') return { handled: true, close: true };
  if (key === 'Enter') return { handled: true, ...(matches[index] ? { select: matches[index].tab_id } : {}) };
  if (key === 'ArrowDown' || key === 'ArrowUp') return { handled: true, index: matches.length ? (index + (key === 'ArrowDown' ? 1 : -1) + matches.length) % matches.length : 0 };
  return { handled: false };
}

export function removeTabbitMentionQuery(value) {
  return String(value ?? '').replace(/(?:^|\s)@[^\s@]*$/u, '').trimEnd();
}

export function methodSelectionState(methodIds = [], nextId = '') {
  const ids = [...new Set(Array.isArray(methodIds) ? methodIds : [])];
  if (!ids.includes(nextId) && ids.length >= 3) return { accepted: false, reason: '最多选择三个方法' };
  return { accepted: true, methodIds: ids.includes(nextId) ? ids.filter(id => id !== nextId) : [...ids, nextId] };
}
...
export function renderQuickSkills(skills = [], { page = 'fingpt', category = '' } = {}) {
  ...
  return `<section class="quick-skills" aria-label="${claw ? '研究步骤模板快捷入口' : '研究 Skill 快捷入口'}"><details class="quick-directory"${selectedCategory ? ' open' : ''}><summary>浏览研究入口与分类</summary><div class="section-heading"><h2>${claw ? '研究步骤模板' : '从真实研究 Skill 开始'}</h2><a href="#/skills" class="text-button">查看能力中心</a></div>${claw ? '<p class="small muted">步骤是研究模板，不代表已经执行；也可通过上方能力选择使用真实 Skill。</p>' : ''}${filters}</details>${cards ? `<div class="quick-skill-grid">${cards}</div>` : `<p class="muted small quick-skill-empty">${claw ? '尚无已启用的研究步骤模板' : '尚无可用研究 Skill'}；请在能力中心检查目录后刷新。</p>`}</section>`;
}

export function renderComposer({ models = [], model = '', page, draft, attachments = [], tabbitTabs = [], tabbitCandidates = [], tabbitOpen = false, tabbitLoading = false, tabbitIndex = 0, expectedFormats = null, skills = [], skillId = '', capability = null, toolIds = [], tools = [], methods = [], methodIds = [], methodPickerOpen = false, disabled = false, busy = false, taskPending = false, detail = null, slashOpen = false, slashIndex = 0, runtimeReady = true, runtimePending = false } = {}) {
  const claw = page === 'claw';
  ...
  const visibleMethods = methodIds.map(id => ({ ...(methods.find(item => item.id === id) || { id, name: id }), method_id: id, selection_source: 'user-selected' }));
  const methodChips = visibleMethods.map(item => `<span>方法：${e(item.name || item.title || item.method_id || item.id)} · ${e(sourceLabels[item.selection_source || item.source] || '用户选择')}<button type="button" data-remove-method="${e(item.method_id || item.id)}" aria-label="移除方法">×</button></span>`).join('');
  const chips = `${capability ? `<span>${e(capability.name)} · v${e(capability.version)}<button type="button" data-clear-capability aria-label="移除所选能力">×</button></span>` : ''}${toolIds.map(id => `<span>工具意图：${e(tools.find(tool => tool.id === id)?.name || id)}<button type="button" data-remove-tool="${e(id)}" aria-label="移除工具意图">×</button></span>`).join('')}${methodChips}`;
  const methodPicker = `<details class="capability-picker method-picker"${methodPickerOpen ? ' open' : ''}><summary aria-label="选择推理方法">方法 ${methodIds.length}/3</summary><fieldset><legend class="sr-only">推理方法，最多三个</legend>${methods.filter(item => item.enabled && item.version).map(item => `<label><input id="method-${e(item.id)}" type="checkbox" data-method-id="${e(item.id)}" ${methodIds.includes(item.id) ? 'checked' : ''} ${!methodIds.includes(item.id) && methodIds.length >= 3 ? 'disabled' : ''}><span>${e(item.name)} · v${e(item.method_spec?.version || item.version)}</span></label>`).join('')}</fieldset></details>`;
  const tabbitMenu = tabbitOpen ? `<div id="tabbit-options" class="slash-menu tabbit-menu" role="listbox" aria-label="可接管的 Tabbit 标签页">${tabbitLoading ? '<p class="small muted" role="status">正在读取可用标签页…</p>' : tabbitCandidates.length ? tabbitCandidates.map((tab, index) => `<button id="tabbit-option-${index}" type="button" role="option" aria-selected="${index === tabbitIndex}" class="slash-skill tabbit-option" data-tabbit-select="${e(tab.tab_id)}"><strong>${e(tab.title || '未命名标签页')}</strong><span>${e(tab.url || '')}</span></button>`).join('') : '<p class="small muted" role="status">没有匹配的可用 HTTP(S) 标签页。</p>'}</div>` : '';
  const tabbitChips = tabbitTabs.length ? `<div class="attachment-chips tabbit-chips" aria-label="已选实时标签页"><span class="chip-count">实时标签页 ${tabbitTabs.length}/8</span>${tabbitTabs.map((tab) => `<span title="${e(tab.url || '')}">${e(tab.title || '未命名标签页')}<button type="button" data-remove-tabbit="${e(tab.tab_id)}" aria-label="移除实时标签页 ${e(tab.title || '')}">×</button></span>`).join('')}</div>` : '';
  const menuOpen = slashOpen || tabbitOpen;
  const controls = tabbitOpen ? 'tabbit-options' : slashOpen ? 'slash-options' : '';
  const active = tabbitOpen && tabbitCandidates.length ? `tabbit-option-${tabbitIndex}` : slashOpen && matches.length ? `slash-option-${slashIndex}` : '';
  const runtimeCaption = runtimePending
    ? '正在连接运行时；可先准备草稿，连接完成后即可发送。'
    : !runtimeReady
      ? '运行时未就绪或离线：无法核对停止；可准备草稿，连接与授权就绪后才能发送。'
      : '';
  return `<form id="composer" class="composer" data-dropzone data-paste-support><label class="sr-only" for="prompt">${claw ? '任务目标或补充信息' : '研究问题'}</label><textarea id="prompt" name="prompt" rows="3" aria-expanded="${menuOpen}" ${controls ? `aria-controls="${controls}"${active ? ` aria-activedescendant="${active}"` : ''}` : ''} placeholder="${claw ? '描述研究目标；输入 @ 引用实时标签页…' : '向 FinGPT 提问；输入 @ 引用实时标签页，/ 选择能力…'}" ${disabled ? 'disabled' : ''}>${e(draft || '')}</textarea>${slashOpen ? `<div id="slash-options" class="slash-menu" role="listbox" aria-label="匹配的研究能力">${matches.length ? matches.map((skill, index) => `<button id="slash-option-${index}" type="button" role="option" aria-selected="${index === slashIndex}" class="slash-skill" data-skill-shortcut="${e(skill.id)}"><strong>${e(skill.name)}${skill.version ? ` · v${e(skill.version)}` : ''}</strong><span>${e(skill.description || '无描述')}</span></button>`).join('') : '<p class="small muted">没有匹配的已启用能力；Escape 关闭。</p>'}</div>` : ''}${tabbitMenu}${chips ? `<div class="attachment-chips capability-chips">${chips}</div>` : ''}${tabbitChips}${attachments.length ? `<div class="attachment-chips">${attachments.map((file) => `<span>${e(file.name)}<button type="button" data-remove-attachment="${e(file.id)}" aria-label="移除附件 ${e(file.name)}">×</button></span>`).join('')}</div>` : ''}<div class="composer-tools"><button type="button" class="button attachment-button" data-upload ${disabled ? 'disabled' : ''} aria-label="添加附件" title="PDF、图片、Markdown、CSV、Excel">${icon('plus')}</button><button type="button" class="button attachment-button" data-slash-search ${disabled ? 'disabled' : ''} aria-label="选择 Skill 或 Workflow" title="搜索真实能力">${icon('grid')} <span>能力</span></button><details class="capability-picker"><summary aria-label="选择已启用能力">${icon('chevron')}</summary><label class="sr-only" for="skill-select">使用 Skill 或 Workflow</label><select id="skill-select" ${disabled ? 'disabled' : ''}><option value="">按需使用能力</option>${enabledSkills.map((skill) => `<option value="${e(skill.id)}" ${skillId === skill.id ? 'selected' : ''}>${e(skill.name)}</option>`).join('')}</select></details>${methodPicker}${renderFormatPicker(expectedFormats, picked)}<span class="spacer"></span><label class="sr-only" for="model-select">运行模型</label><select id="model-select" ${busy ? 'disabled' : ''}>${modelOptions(models, model)}</select>${detail && (isRunning(detail.status) || detail.can_cancel || detail.can_recheck_stop) ? `<button type="button" class="button danger-outline" data-cancel ${busy || !runtimeReady ? 'disabled' : ''}>${detail.can_recheck_stop ? '重新核对停止' : '停止'}</button>` : ''}<button type="submit" class="button primary composer-send" aria-label="${busy ? '正在提交' : '发送研究问题'}" title="${detail ? '发送' : '开始研究'}" ${disabled || taskPending || !runtimeReady ? 'disabled' : ''}>${icon('arrow')}</button></div><input id="file-input" type="file" multiple accept=".pdf,.png,.jpg,.jpeg,.webp,.gif,.md,.csv,.xlsx" hidden></form><p class="composer-caption">${runtimeCaption || (taskPending ? '上一任务及其交付尚未确认结束；可准备草稿，完成后再发送。' : 'Enter 发送 · Shift + Enter 换行 · @ 实时标签页 · / 选择能力')}</p>`;
}
