...
export function renderQuickSkills(skills = [], { page = 'fingpt', category = '' } = {}) {
  ...
  const cards = visible.map((skill) => `<article class="quick-skill-card"><button type="button" class="quick-skill-title" data-skill-detail="${e(skill.id)}">${icon(({ 'document-reading': 'document', 'company-research': 'company', 'industry-research': 'industry', 'fund-evaluation': 'chart' })[skill.id] || 'layers')}<h3>${e(skill.name)}</h3></button><p class="quick-input">${e(claw ? '步骤式研究模板' : (skill.metadata?.inputs || []).map(input => input.label).join('、') || '研究问题与资料')}</p><span class="sr-only">${e(skill.description || '')} 场景：${e((skill.metadata?.scenarios || []).join('、') || '未提供')} 输入：${e((skill.metadata?.inputs || []).map(input => `${input.label}（${input.type}${input.required ? ' · 必填' : ''}）`).join('、') || '未提供')} 输出：${e((skill.metadata?.default_formats || []).join('、') || '无需文件')}</span><button type="button" class="quick-use icon-button" data-skill-shortcut="${e(skill.id)}" aria-label="将${e(skill.name)}放入草稿" title="放入草稿，不会发送" ${skill.enabled === false ? 'disabled' : ''}>${icon('arrow')}</button></article>`).join('');
