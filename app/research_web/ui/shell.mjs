...
export function renderPrimaryRail({ page, secondaryOpen = false } = {}) {
  return `<nav class="primary-rail" aria-label="产品主导航"><a class="rail-brand" href="#/fingpt" aria-label="AlphaFoundry Research">A</a><div class="primary-nav">${navItems.map(([target, icon, title]) => `<a href="#/${target}" class="rail-link ${page === target ? 'active' : ''}" ${page === target ? 'aria-current="page"' : ''} title="${e(title)}"><span aria-hidden="true">${icon}</span><span class="rail-label">${e(title)}</span></a>`).join('')}</div><button class="rail-link rail-toggle ${secondaryOpen ? 'active' : ''}" data-toggle-sidebar aria-label="${secondaryOpen ? '关闭会话侧栏' : '打开会话侧栏'}" aria-expanded="${secondaryOpen}">☰</button><a class="rail-link rail-settings ${page === 'settings' ? 'active' : ''}" href="#/settings" title="设置"><span aria-hidden="true">⚙</span><span class="rail-label">设置</span></a></nav>`;
}
...
export function renderTopbar({ runtimeLabel, runtime, models = [], busy = false, search = '', sessions = [], skills = [], searchOpen = false } = {}) {
  return `<header class="topbar ${searchOpen ? 'search-open' : ''}"><button class="icon-button menu-toggle" data-toggle-sidebar aria-label="打开导航">☰</button><a class="brand" href="#/fingpt"><img src="/static/assets/alphafoundry-logo.png" width="30" height="30" alt=""><span>AlphaFoundry</span><span class="brand-divider"></span><span class="brand-label">Research</span></a><button type="button" class="icon-button mobile-search-toggle" data-toggle-search aria-label="${searchOpen ? '关闭全局搜索' : '打开全局搜索'}" aria-expanded="${searchOpen}">⌕</button>${renderGlobalSearch(search, sessions, skills)}<div class="topbar-right"><a href="#/settings" class="runtime-status"><span class="tiny-dot ${runtime?.connected ? 'active' : ''}"></span>${e(runtimeLabel || 'DSH 未连接')}</a><label class="sr-only" for="model-select">运行模型</label><select id="model-select" ${busy ? 'disabled' : ''}>${modelOptions(models, runtime?.model)}</select><button class="icon-button" data-refresh aria-label="刷新服务状态" title="刷新服务状态">↻</button></div></header>`;
}
