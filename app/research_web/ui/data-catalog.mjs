...
function capabilityCard(item, busy) {
  ...
  return `<article class="data-card capability-data-card">
    ...
    <div class="card-state">${badge(ready ? '可通过 DataHub 调用' : '暂无已适配来源', ready ? 'live' : 'danger')}<div class="button-row"><button class="button small" data-data-capability-detail="${e(item.id)}" data-cap-preview-trigger="data:${e(item.id)}">查看来源矩阵</button><button class="button small primary" data-use-data-tool="${e(item.tool_id)}" ${busy || !ready ? 'disabled' : ''}>放入研究草稿</button></div></div>
  </article>`;
}
...
export function renderDataCatalog({ catalog = {}, view = 'capabilities', query = '', category = '', market = '', status = '', auth = '', busy = false, error = '', probe = null, showViewSwitch = true } = {}) {
