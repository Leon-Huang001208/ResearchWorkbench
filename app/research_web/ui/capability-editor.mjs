...
export function newDraft(kind = 'skill') {
  return { kind, metadata: { name: '', slug: '', description: '', category: '', inputs: [{ name: 'question', label: '研究问题', type: 'text', required: true }], scenarios: [], default_formats: ['md'], required_tools: [], dependencies: [], method_policy: { required: [], recommended: [], excluded: [] } }, instructions: '', files: [], steps: kind === 'workflow' ? [newWorkflowStep()] : [], reviewed_scripts: [] };
}
...
export function moveStepWithFeedback(steps, index, direction) {
  const current = array(steps);
  const target = index + direction;
  if (!Number.isInteger(index) || ![-1, 1].includes(direction) || index < 0 || target < 0 || index >= current.length || target >= current.length) {
    return { steps: jsonCopy(current), focusId: '', announcement: '' };
  }
  const moved = current[index];
  const result = moveStep(current, index, direction);
  const title = text(moved.title) || `步骤 ${index + 1}`;
  return {
    steps: result,
    focusId: `cap-step-${moved.client_key}-move-${direction < 0 ? 'up' : 'down'}`,
    announcement: `${title}已移动到第 ${target + 1} 步，共 ${result.length} 步`,
  };
}
...
export function readEditor(values, previous) {
  ...
  return { kind: previous.kind, metadata: { name: get('name'), slug: get('slug'), description: get('description'), category: get('category'), inputs, scenarios: lines(get('scenarios')), default_formats: values.getAll('default_formats').map(String), required_tools: values.getAll('required_tools').map(String), dependencies: lines(get('dependencies')), method_policy: { required: lines(get('method-required')), recommended: lines(get('method-recommended')), excluded: lines(get('method-excluded')) } }, instructions: get('instructions'), files,
...
export function renderCapabilityEditor({ draft, id = '', items = [], tools = [], busy = false, announcement = '' } = {}) {
  if (!draft) return '';
