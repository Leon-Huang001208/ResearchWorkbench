...
export function editableDraft(draft = {}) {
  return { kind: draft.kind || 'skill', metadata: jsonCopy(draft.metadata || {}), instructions: text(draft.instructions), files: array(draft.files).map(file => ({ path: file.path, ...(typeof file.content === 'string' ? { content: file.content } : { base64: file.base64 }) })), steps: jsonCopy(array(draft.steps)), reviewed_scripts: [...array(draft.reviewed_scripts)] };
}

export function newDraft(kind = 'skill') {
  return { kind, metadata: { name: '', slug: '', description: '', category: '', inputs: [{ name: 'question', label: '研究问题', type: 'text', required: true }], scenarios: [], default_formats: ['md'], required_tools: [], dependencies: [] }, instructions: '', files: [], steps: kind === 'workflow' ? [{ title: '', instruction: '', skill_id: null, tools: [] }] : [], reviewed_scripts: [] };
}
...
export function readEditor(values, previous) {
  ...
  return { kind: previous.kind, metadata: { name: get('name'), slug: get('slug'), description: get('description'), category: get('category'), inputs, scenarios: lines(get('scenarios')), default_formats: values.getAll('default_formats').map(String), required_tools: values.getAll('required_tools').map(String), dependencies: lines(get('dependencies')) }, instructions: get('instructions'), files,
    steps: array(previous.steps).map((_step, index) => ({ title: get(`step-${index}-title`), instruction: get(`step-${index}-instruction`), skill_id: get(`step-${index}-skill`) || null, tools: values.getAll(`step-${index}-tools`).map(String) })),
